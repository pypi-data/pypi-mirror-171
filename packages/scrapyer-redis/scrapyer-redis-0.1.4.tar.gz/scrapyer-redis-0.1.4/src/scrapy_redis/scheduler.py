...
class StreamScheduler(Scheduler):
    ...
    def next_request(self):
        ...
        block_pop_timeout = self.idle_before_close # 等待关闭时间
        ...
        if request and self.stats:
            ...
            if self.spider.count_size(self.spider.redis_key): 
                self.waiting = False
                logger.info(
                    "Moving start URLs from redis key "
                )
                for req in self.spider.next_requests():
                    self.enqueue_request(req)
                    # logger.info('Running request {}'.format(req.url))
                # return request
            else:
                if not self.waiting:
