...
class GPmodel_systematics
{
    private:
        ...
        // The signal
        vector<double> mu = vector<double>(data.N());
        // The covariance matrix for the data
        Eigen::MatrixXd C {data.N(), data.N()};
        Eigen::MatrixXd Cx {data.N(), data.N()};
        vector<double> sorted_actind = vector<double>(data.N());
    ...
    public:
        ...
        /// Prior for $\eta_1^x$, the GP "amplitude"
        std::shared_ptr<DNest4::ContinuousDistribution> eta1x_prior;
        /// Prior for $\eta_2^x$, the GP correlation timescale
        std::shared_ptr<DNest4::ContinuousDistribution> eta2x_prior;
        ...
        void calculate_C();
        /// @brief Build the second covariance matrix
        void calculate_Cx();
        void sort_actind();
