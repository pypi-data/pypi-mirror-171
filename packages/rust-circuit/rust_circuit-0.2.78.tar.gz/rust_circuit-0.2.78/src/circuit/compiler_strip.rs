...
use super::{
    deep_map_op_context, deep_map_unwrap, visit_circuit, Circuit, CircuitNode, CircuitRc,
    HashBytes, ModuleNodeArgSpec,
};
use crate::circuit::{ModuleNode, ModuleNodeSpec};
...
pub fn strip_names(circuit: CircuitRc, context: &mut OptimizationContext) -> CircuitRc {
    let result = deep_map_op_context(
        &circuit,
        &|circuit, _| match circuit {
            Circuit::Symbol(_sym) => None,
            _ => circuit.name().map(|_| circuit.clone().rename(None).rc()),
        },
        &mut (),
