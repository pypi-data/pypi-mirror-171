...
def generate_chart(input_args):
    ...
    diagram_input_data = list(collection_data_for_plot(data_files, parse(input_args.timestamp), input_args.time_range))
    ...
    for file in data_files:
        if file.endswith("gz"):
            with gzip.open(file, mode="rt") as data_f:
                ...
                res = (res[0] + one_file_res[0], res[1] + one_file_res[1], res[2] + one_file_res[2])
        else:
...
def init_input_args(sub_parser):
    ...
    sub_parser.add_argument("--time_range", type=int, help="产品类型：分钟，截取的日志范围，距离时间点（--timestamp）前后多少分钟。默认1分钟", choices=range(1, 6), required=True)
    sub_parser.add_argument("--log_root_dir", help="log 文件的根目录", required=True)
    sub_parser.add_argument("--timestamp", help="时间点, 例如：2022-09-05 09:53:10。 默认为当前时间", default=datetime.now().strftime("%m-%d-%Y %H:%M:%S"))
    sub_parser.add_argument("--robot_id", help="目标机器人的编号，例如：F48A57S", required=True)
    sub_parser.add_argument("--output_file", help=f"输出文件的路径, 默认路径：{OUTPUT_FILE}", default=OUTPUT_FILE)
