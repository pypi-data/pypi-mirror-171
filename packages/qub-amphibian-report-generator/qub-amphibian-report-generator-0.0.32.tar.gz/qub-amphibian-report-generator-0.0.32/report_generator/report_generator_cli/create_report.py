...
def create_title_page(
    ...
    with pdf.local_context(
        ...
        if "title_sub_font" in font_options.keys():
            ...
            title_sub_font_size = font_options["title_sub_size"]
        else:
            title_sub_font = font_options["title_font"]
            title_sub_font_size = font_options["title_size"]
...
def create_report_order_sections(ds: object, pdf: object, config: dict, font_options: dict) -> object:
...
def create_report_section_pages(section, pdf, config, font_options):
    ...
    for i in range(len(amp_list)):
        ...
        if report_style == "compact":
            ...
            if i % 3 == 2:
                image_offset += 165
                pdf = create_report_page_compact(amp, pdf, image_offset, config, font_options)
                pdf.add_page()
            elif i % 3 == 1:
                image_offset += 85
                pdf = create_report_page_compact(amp, pdf, image_offset, config, font_options)
...
def create_report_page_compact(amp: object, pdf, image_offset, config, font_options) -> object:
