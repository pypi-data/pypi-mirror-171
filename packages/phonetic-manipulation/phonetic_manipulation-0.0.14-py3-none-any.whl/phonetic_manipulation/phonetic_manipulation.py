...
class Phonetic_Manipulation():
	...
	def from_IPA(self, word: str) -> str:
		...
		skip = False
		if '&' in word:
			if re.search(f'{pm.vowels}&{pm.vowels}', word):
				letter = re.search(f'{pm.vowels}&{pm.vowels}', word)[0]
				for l in letter.split('&'):
					i = pm.ipa_p.index(l)
					diphthong.append(pm.ipa_x[i])
				diphthong = ''.join(diphthong)
				d_x = word.index(letter[-1])+1
			if re.search(f'{pm.consonants}&{pm.consonants}', word):
				letter = re.search(f'{pm.consonants}&{pm.consonants}', word)[0]
				for l in letter.split('&'):
					i = pm.ipa_p.index(l)
					affricate.append(pm.ipa_x[i])
				affricate = ''.join(affricate)
				a_x = word.index(letter[-1])+1
		for letter in word:
			if not skip:
				if letter in pm.dm_p:
					i = pm.dm_p.index(letter)
					new_word.append(pm.dm_x[i])
					continue
				if letter in pm.ipa_p:
					i = pm.ipa_p.index(letter)
					new_word.append(pm.ipa_x[i])
