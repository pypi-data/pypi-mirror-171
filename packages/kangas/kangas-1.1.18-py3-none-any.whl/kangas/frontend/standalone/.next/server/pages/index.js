...
if (external_process_.env.KANGAS_BACKEND_PROXY) {
    localConfig.apiUrl = `${external_process_.env.KANGAS_BACKEND_PROXY}/datagrid/`;
} else {
    localConfig.apiUrl = `http://${external_process_.env.KANGAS_HOST}:${external_process_.env.KANGAS_BACKEND_PORT}/datagrid/`;
}
...
const Root = ({ query , matrices  })=>{
    /* eslint-disable no-unused-vars */ const { data: table , tableError  } = useData(`query-${lib_hashQuery({
        query
