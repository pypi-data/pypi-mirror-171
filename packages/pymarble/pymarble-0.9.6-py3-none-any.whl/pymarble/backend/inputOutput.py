  ...
  with open(pyFile, 'w') as fOut:
    ...
    for start in self.content:
      if 'start' in self.periodicity and start==int(self.periodicity['start']):
        ...
        fOut.write('    hdfBranch = fOut.create_group("Test"+str(idxTest+1))\n')
        inForLoop = True
      sect= self.content[start]
      if not sect.important:  #don't output unimportant items
        continue
      branchName = 'hdfBranch' if sect.dClass in ['meta','primary'] and inForLoop else None
...
convURI = 'https://raw.githubusercontent.com/NFDI4Chem/formaTAPIRest/main/cwl/"""+self.meta['ext']+"""2hdf.cwl'
convVersion = '1.0'
