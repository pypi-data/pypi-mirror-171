...
def processCustomTransactionOnChain(block,name,parameters:dict,pool:dict):
    ...
    if name == 'addpoolliquidity':
        ...
        (poolFrom,poolTo,swapped,dexFeeInAmount,dexFeeOutAmount) = chain.poolSwap(block,parameters['fromAmount'],parameters['isForward'],pool['reserveA'],pool['reserveB'],parameters['commission'],parameters['dexFeeInPct'],parameters['dexFeeOutPct'])
        return {
            'success': 1,
