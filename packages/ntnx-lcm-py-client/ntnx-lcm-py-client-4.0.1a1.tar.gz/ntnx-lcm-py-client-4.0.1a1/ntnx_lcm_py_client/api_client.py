...
import ntnx_lcm_py_client.Ntnx
from ntnx_lcm_py_client import rest
...
class ApiClient(object):
    ...
    def add_default_header(self, header_name, header_value):
        """A method to add default headers to each HTTP request
        """
    ...
    def __get_request_timeout(self, request_timeout):
        ...
        :param request_timeout: The request timeout in milliseconds provided by individual operation
        """

        max_timeout_in_seconds = 30 * 60
        if request_timeout and request_timeout > 0:
            return min(request_timeout/1000, max_timeout_in_seconds)
        else:
            if self.configuration.timeout and self.configuration.timeout > 0:
                return min(self.configuration.timeout/1000, max_timeout_in_seconds)
            else:
                return self.configuration.default_timeout/1000
    ...
    def __call_api(
        ...
        # request url
        url = 'https://' + config.host + ':' + str(config.port) + resource_path
        ...
        if response_data.status == 401:
            ...
            if response_data.status == 401:
                raise ntnx_lcm_py_client.rest.ApiException(http_resp=response_data)
        ...
        if _preload_content:
            ...
            if response_data.status is not 204:
    ...
    def deserialize_response(self, response, response_type):
    ...
    def deserialize(self, data, klass, discriminator = None):
        ...
        :param str discriminator: Needed in case of a one of object
        :return: object.
        """ # noqa: E501
        if data is None:
            return None
        ...
        if klass in self.PRIMITIVE_TYPES:
            ...
            return self.__deserialize_model(data, klass, discriminator)
    ...
    def call_api(self, resource_path, method,
        ...
        if not async_req:
            ...
            thread = self.__pool.apply_async(self.__call_api, (resource_path,
                                           ...
                                           collection_formats,
                                           _preload_content, client_request_timeout))
            return thread
    ...
    def get_etag(object):
        ...
        :param response: object The object from which etag needs to be fetched
        """ # noqa: E501
        etag = None
        if object:
    ...
    def __getattr(self, name):
        if name in self.DUPLICATE_SCHEMA_MAPPING:
            return getattr(ntnx_lcm_py_client.Ntnx, self.DUPLICATE_SCHEMA_MAPPING[name])
        if hasattr(ntnx_lcm_py_client.Ntnx, name.split('.')[-1]):
            return getattr(ntnx_lcm_py_client.Ntnx, name.split('.')[-1])
        else:
            return getattr(ntnx_lcm_py_client.Ntnx, name)

    def __deserialize_model(self, data, klass, discriminator = None):
            ...
            for attr, attr_type in six.iteritems(klass.swagger_types):
                if data is not None and isinstance(data, (list, dict)):
                    ...
                    if attr_type.startswith('OneOf'):
                        attr_type = self.__getattr(attr_type)
                        if attr_type.ONE_OF_ITEM_DISCRIMINATOR_NAME in data:
                            disc = data[attr_type.ONE_OF_ITEM_DISCRIMINATOR_NAME]
                            data.pop(attr_type.ONE_OF_ITEM_DISCRIMINATOR_NAME)
                    kwargs[attr] = self.deserialize(value, attr_type, disc)
        ...
        if self.__hasattr(instance, 'get_real_child_model'):
            if discriminator is not None:
                klass_name = instance.get_real_child_model(discriminator)
                if klass_name:
