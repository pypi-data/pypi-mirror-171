...
class Enrichments:
    ...
    def available_enrichments(self):
        return dict(
            ...
            fetch_descendants=self.fetch_descendants,
    ...
    def fetch_descendants(
        ...
        flat_dict_titles = self.fetch_flat_dict_for_field(
            name=name, field="title", steps=steps, verbose=verbose, conservative=False
        )
        list_descendants = list(flat_dict_titles.keys())
        if len(list_descendants) > 0:
    ...
    def fetch_available(
        ...
        defaults = self.fetch_defaults(name=name, verbose=verbose)
        descendants = self.fetch_descendants(name=name, verbose=verbose)

        available = dict()
        if isinstance(descendants, list):
            for k in descendants:
                if defaults is not None:
                    available[k] = defaults.get(k, None)
                else:
                    available[k] = None
        else:
            pass
