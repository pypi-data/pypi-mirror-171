    ...
    def run(
        ...
        for frame in video.frames:
            if not frame.detections:
                continue  # skip empty frames
            frame_path = os.path.join(self.root_dir, "JPEGImages", video.video_id, f"{frame.number}.jpg")
            ...
            for detection in frame.detections:
                if detection.type == VideoDetectionType.BOUNDING_BOX and isinstance(
                    ...
                    logger.debug(f"Unsupported Detection Type: {detection.type} for video {video.video_id} for pascalVoc format.. Skipping")
                    continue

            annotations_root = os.path.join(self.root_dir,'Annotations', video.video_id)
            segmentation_root = os.path.join(self.root_dir, 'SegmentationClass', video.video_id)
            os.makedirs(annotations_root, exist_ok=True)
            os.makedirs(segmentation_root, exist_ok=True)
            # save annotation
            pascal_voc_frame_annotation.save(os.path.join(annotations_root, frame.frame_id+'.xml'))
            # save segmentation mask if any
            if segmented:
                segmentation = PIL.Image.fromarray(mask)
                segmentation.putpalette(pascal_voc_color_map())
                segmentation.save(os.path.join(segmentation_root,frame.frame_id+'.png'))
...
class PascalVocExporter:
    ...
    def export(self, folder: deeplabel.label.folders.RootFolder, write_frames:bool=False):
        ...
        if folder.type == deeplabel.label.folders.FolderType.VIDEO:
            exporter = VideoExporter(root_dir=self.root_dir, client=self.client, label_to_int=label_to_int, write_frames=write_frames)
