...
#         s += '\n'
    ...
    def getPhysicalIndices(self, start, stop):
        start = self.logical2physicalTagStart(start)
        stop = self.logical2physicalTagEnd(stop - 1) + 1
        return start, stop
...
class XMLNodeText(XMLNode):
    ...
    def logical2physicalTagStart(self, pos):
        return pos

    logical2physicalTagEnd = logical2physicalTagStart
...
class XMLNodeBinaryTag(XMLNode):
    ...
    def logical2physicalTagStart(self, pos):
        pos = self.arg.logical2physicalTagStart(pos)
        if pos:
            pos += self.tagBegin.length
        return pos
    
    def logical2physicalTagEnd(self, pos):
        pos = self.arg.logical2physicalTagEnd(pos)
        if pos:
            if pos == len(str(self.arg)) - 1:
                pos += self.tagEnd.length
            pos += self.tagBegin.length
        return pos
...
class XMLNodeArray(XMLNode):
    ...
    def logical2physicalTagStart(self, pos):
        ...
        index = binary_search(logicalOffset, pos, lambda args, hit: -1 if hit >= args[1] else 1 if hit < args[0] else 0)
        prev_start = logicalOffset[index][0]
        return self.args[index].logical2physicalTagStart(pos - prev_start) + prev_start + offsets[index]
    
    def logical2physicalTagEnd(self, pos):
        logicalOffset, offsets = self.logicalOffset, self.offsets
        index = binary_search(logicalOffset, pos, lambda args, hit: -1 if hit >= args[1] else 1 if hit < args[0] else 0)
        prev_start = logicalOffset[index][0]
        return self.args[index].logical2physicalTagEnd(pos - prev_start) + prev_start + offsets[index]
...
def construct_rich_text(text):
    start = 0
    
    richTexts = []
    leftTagCount = {} 
    for m in re.finditer(r'''<([a-z][-:_a-z]*\d*)(?:\s+:?[a-z][-:_a-z]*=(?:"[^"]*"|'[^']*'))*\s*>(?=[\s\S]*?<\/\1>)|</([a-z][-:_a-z]*\d*)>|<(img|mspace|br)(?:\s+:?[a-z][-:_a-z]*=(?:"[^"]*"|'[^']*'))*\s*/>|&(#[0-9]+|#x[0-9a-f]+|[^\t\n\f <&#;]{1,32});''', text, re.IGNORECASE):
#                             1-----------------)                                                                     2-----------------)   3-------------)                                                   4----------------------------------------)
