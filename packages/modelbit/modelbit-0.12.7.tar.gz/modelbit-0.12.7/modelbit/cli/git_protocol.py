import logging
import sys
from typing import BinaryIO, Callable, List, NoReturn, Optional, Tuple
...
class GitProtocolBase:
  ...
  def __init__(self, inStream: BinaryIO = sys.stdin.buffer, outStream: BinaryIO = sys.stdout.buffer):
...
class GitProtocol(GitProtocolBase):
  ...
  def versionHandshake(self) -> None:
    ...
    self.write(toPkt(b"git-filter-server\n"))
    self.write(toPkt(b"version=2\n"))
    self.write(FLUSH_PACKET)

  def capabilitiesHandshake(self) -> None:
    caps = self.readKVData()
    # TODO: Process capabilities
    self.write(toPkt(b"capability=clean\n"))
    self.write(toPkt(b"capability=smudge\n"))
    # TODO: Support delay to allow parallel file downloads
    # self.write(toPkt(b"capability=delay\n"))
    self.write(FLUSH_PACKET)
  ...
  def writeSuccess(self, content: Optional[bytes]) -> None:
    self.write(toPkt(b"status=success\n"))
    self.write(FLUSH_PACKET)
    if content and len(content):
      self.write(toPkt(content))
    self.write(FLUSH_PACKET)
    self.write(FLUSH_PACKET)

  def writeError(self) -> None:
    self.write(toPkt(b"status=error\n"))
    self.write(FLUSH_PACKET)
...
def toPkt(s: bytes) -> bytes:

  size = len(s)
  if size <= 65516:
    return b"%04x%s" % (size + 4, s)

  return toPkt(s[:61156]) + toPkt(s[61156:])
