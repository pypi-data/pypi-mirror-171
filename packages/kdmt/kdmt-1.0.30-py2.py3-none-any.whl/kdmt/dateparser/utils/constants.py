...
MONTHS_PATTERN = r"\b(january|februar(y|i)|march|april|june|july|august(us)?|september|october|november|december|enero|febrero|marzo|abril|mayo|junio?|julio?|agosto|septiembre|octubre|noviembre|diciembre|januar|februar|marts|april|maj|september|oktober|november|december|janv?[\.\s]|ene[\.\s]|feb[\.\s]|mar[\.\s]|apr[\.\s]|abr[\.\s]|may[\.\s]|maj[\.\s]|jun[\.\s]|jul[\.\s]|aug[\.\s]|ago[\.\s]|sep[^A-Za-z]|sept[\.\s]|oct[\.\s]|okt[\.\s]|nov[\.\s]|d(e|é)c[\.\s]|dic[\.\s]|janvier|f(e|é)v(rier)|mars|maart|avril|mai|mei|juin|juillet|aout|août|septembre|octobre|novembre|d(e|é)cembre)\b"
...
DELIMITERS_PATTERN = r"[/\:\-\,\.\s\_\+\@]+|(?<=\d\s*)h\s*|(?<=\d)m(?<=\d)s"
TIME_PERIOD_PATTERN = r"a\.m\.|am|p\.m\.|pm"
## can be in date strings but not recognized by dateutils
EXTRA_TOKENS_PATTERN = r"\b(due|by|on|à|om|during|standard|daylight|savings|time|date|dated|of|to|through|between|until|at|day)\b"
