...
class StandardDatasetOp(BaseDatasetOp):
    ...
    def parent(self) -> st.Dataset:
        return parent(self.dataset)
...
class StandardScalarOp(BaseScalarOp):
    def parent(self) -> st.DataSpec:
        return parent(self.scalar)
...
def parent(dataspec: st.DataSpec) -> st.Dataset:
    pars = parents(dataspec)
    parent = [element for element in pars if isinstance(element, Dataset)]
    assert len(parent) == 1
    return parent[0]
