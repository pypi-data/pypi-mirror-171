...
class results_analysis():
    ...
    def __init__(self, solution, labels, convert=False, origID=None, species='human'):
        ...
        if len(isec)>0:
            ...
            self.isec = list(isec)
            self.isec_entr = [str(self.labels[x]) for x in self.isec]
        ...
        if convert == True:
            ...
            if len(self.isec)>0:
                all_genes = all_genes + self.isec_entr
    ...
    def show_networks(self, GE, G, output=None):
        ...
        if len(self.isec)>0:
            all_genes_entr = all_genes_entr + self.isec_entr
        ...
        if self.convert:
            ...
            if len(self.isec) > 0:
                isec_name = [self.mapping[x] for x in self.isec_entr]
    ...
    def show_clustermap(self, GE, G, true_labels=None, output=None, class_names = []):
        ...
        if self.convert:
            ...
            if len(self.isec) > 0:
                all_genes_names = genes1_name + genes2_name + self.isec_entr
            else:
