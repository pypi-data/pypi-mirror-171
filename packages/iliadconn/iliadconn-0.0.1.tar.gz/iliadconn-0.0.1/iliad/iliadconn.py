...
def calc_forecast(date_next_subscription, consumed, total_max, maintenance):
    date_prev_subscription = date_next_subscription.replace(month=date_next_subscription.month - 1)
    days_passed = (datetime.now().date() - date_prev_subscription.date()).days or 1
    days_next = (date_next_subscription.date() - datetime.now().date()).days
    ...
    with open(os.path.join(tempfile.gettempdir(), TMP_DEBUG_FILE), 'w') as f:
        f.write(json.dumps({'avg_consumed_day': avg_consumed_day,
                            ...
                            'days_next': days_next,
                            'date_prev_subscription': date_prev_subscription.isoformat(),
                            'date': datetime.now().isoformat()}))
