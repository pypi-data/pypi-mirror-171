...
@LastEditTime: 2022-06-09 14:46:01
...
class BindSpecialGoodsHandler(ClientBaseHandler):
    ...
    def get_async(self):
        ...
        goods_base_model = GoodsBaseModel(context=self)
        app_key, app_secret = self.get_app_key_secret()
        invoke_result_data = goods_base_model.bind_special_goods(app_id, goods_id, goods_name, access_token, app_key, access_token, is_log)
        if invoke_result_data.success == False:
            return self.response_json_error(invoke_result_data.error_code, invoke_result_data.error_message)
