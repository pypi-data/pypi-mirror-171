from vayaAuto import __BASE__, baseConfig
...
class VayaDrive(object):
    ...
    def __init__(self, vaya_dir_path, console=False):
        ...
        self.compiled = False
        self.configuration = {}
        if self.vaya_dir_path in baseConfig.CONFIGURATION.keys():
            self.configuration = copy.deepcopy(baseConfig.CONFIGURATION[self.vaya_dir_path])
    ...
    def set_exe_path(self, root_path):
        ...
        if os.path.isfile(os.path.join(root_path, build_folder, 'Release', exe)):
            # exe_path = os.path.join(root_path, build_folder, 'Release', exe)
            self.exe_path = os.path.join(root_path, build_folder, 'Release', exe)
            logger.info(f'Found exe1 - {self.exe_path}')
            return
        if self.consoleMode:
            logger.info(f'Old build system search VayaDriveConsole in build_Release folder')
            build_folder = 'build_Release'
            if os.path.isfile(os.path.join(root_path, build_folder, 'Release', exe)):
                self.exe_path = os.path.join(root_path, build_folder, 'Release', exe)
                logger.info(f'Found exe2 - {self.exe_path}')
                return
        else:
            path = self.find_vd_exe_path(root_path, posix)
            if os.path.isfile(os.path.join(path, exe)):
                self.exe_path = os.path.join(root_path, build_folder, 'Release', exe)
                logger.info(f'Found exe3 - {self.exe_path}')
                return
        logger.error(f'Not found exe - {self.exe_path}')
        raise FileNotFoundError(f'found exe - {self.exe_path}')
    ...
    def _set_vaya_config(self):
        ...
        with open(self.temp_ini_file_path, "r") as f:
            lines = f.readlines()
            if len(lines) == 0:
                raise VayaException(f'{"".join(self.process_output)}\n')
        ...
        baseConfig.CONFIGURATION[self.vaya_dir_path] = copy.deepcopy(self.configuration)
        self.process_output = []
