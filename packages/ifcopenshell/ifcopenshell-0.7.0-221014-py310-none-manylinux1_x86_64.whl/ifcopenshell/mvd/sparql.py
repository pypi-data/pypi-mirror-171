...
class builder(object):
    ...
    def __repr__(self):
        ...
        return "\n".join(itertools.chain((g(s) for s in self.prefixes.items()), (f(s) for s in self.statements)))
...
class ifcOwl(object):
    ...
    def name(S, entity, attribute):
        ...
        while True:
            ...
            attribute_names = ifcOwl.get_names(en, "all_attributes") | ifcOwl.get_names(en, "all_inverse_attributes")

            if st:
                attribute_names -= ifcOwl.get_names(st, "all_attributes") | ifcOwl.get_names(
                    st, "all_inverse_attributes"
                )
        ...
        def build(rule, parents):
            ...
            if rule.tag == "EntityRule":
                ...
                if invattr:
                    ...
                    #     inventy
                    # )
                    bld.append(INDENT + next_nm, invattr, rule_mapping[parents[-1]].name)
                else:
                    bld.append(
...
class executor(object):
    ...
    def run(CR, fn, ttlfn):
        ...
        for ci, C in enumerate(concept_enumerator):
            ...
            print(
                tabulate.tabulate(
                    list(map(dict_to_list(hd), [r for r in roots if r["GlobalId"] not in passing_guids])),
                    hd,
                    tablefmt="grid",
                )
            )
