...
@dataclass
class CollaboraterCompleteEvent(ContractEvent):
    """An event of notify a collaborater completes its task."""
    ...
    def contract_to_event(cls, contract: dict) -> 'CompleteTaskEvent':
        ...
        assert event_type == cls.TYPE, f'合约类型错误: {event_type}'
        assert peer_id and isinstance(peer_id, str), f'invalid peer_id: {peer_id}'
        return CollaboraterCompleteEvent(peer_id=peer_id)
...
class HeteroNNEventFactory(RSAEventFactory):
    ...
    _CLASS_MAP = {
        ...
        CollaboraterCompleteEvent.TYPE: CollaboraterCompleteEvent,
...
class HeteroNNContractor(RSAContractor):
    ...
    def notify_collaborater_complete(self, peer_id: str, host: str):
        """Notify the host a collaborater completes its task."""
        event = CollaboraterCompleteEvent(peer_id=peer_id)
        self._new_contract(targets=[host], event=event)
