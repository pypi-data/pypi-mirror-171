...
app.logger.info("Starting S3Manger with name {APP_NAME}.".format(APP_NAME=app.config["APP_NAME"]))
...
def run():
    arParser = S3ServerAroParser()
    args = arParser.parser.parse_args()
    serve(app, **arParser.getKw(args))
