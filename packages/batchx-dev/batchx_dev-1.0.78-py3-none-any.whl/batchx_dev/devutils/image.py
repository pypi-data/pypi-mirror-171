...
def default_rule_parameters(
    ...
    matching_keys = [k for k in config["tools"].keys() if name in k]

    if name in config["tools"].keys():
        ...
        """.format(
            n=name, k=set(config["tools"].keys())
        )
        print(msg)
        pass

    configuration = config["tools"].get(name, dict()).copy()
