...
class OTFFunctionWriter(object):
    ...
    @staticmethod
    def convert_df_to_list_of_lists(df: pd.DataFrame):
...
def {function_name}(self, {params}) -> TemplateInstance:
    ...
    for param_name, nested_template in {{k: v for k, v in locals().items() if isinstance(v, TemplateInstance) or isinstance(v, pd.DataFrame)}}.items():
