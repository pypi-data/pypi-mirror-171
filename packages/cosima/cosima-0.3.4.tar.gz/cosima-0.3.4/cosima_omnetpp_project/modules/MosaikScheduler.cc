...
void MosaikScheduler::sendToMosaik(cMessage *reply) {
    ...
    if (scheduleMessage) {
        if(pmsg_group.current_mosaik_step() == currentStep) {
            ...
            endOfStepEvent->setArrival(schedulerModule->getId(), -1, currentStep_d+0.00001);
            getSimulation()->getFES()->insert(endOfStepEvent);
            log("MosaikScheduler: insert event 'send message to mosaik' for time " + std::to_string(currentStep_d+0.00001) );
