...
class _AppleDevice(DeviceMeta):
    def __init__(self, device_url):
        ...
        if device_url.startswith('http'):
            ...
            port = get_free_port()
            logger.info("ios connect: %s: %s", device_url, str(port))
            c = wda.USBClient(device_url, port)
        else:
            c = wda.USBClient()
    ...
    def check_wda(self):
        try:
            ...
            if not self.__device_url.startswith('http') and not self.__device_url == "":
                port = get_free_port()
                logger.info("ios connect: %s: %s", self.__device_url, str(port))
                c = wda.USBClient(self.__device_url, port)
                self._client = c
                self.__scale = c.scale
