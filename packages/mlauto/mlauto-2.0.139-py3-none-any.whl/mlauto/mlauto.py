...
def tracefunc(frame, event, arg, indent=[0]):
    if 'my' in frame.f_code.co_name or 'add' in frame.f_code.co_name:
...
def tracefunc(frame, event, arg, indent=[0]):
    l = ['oread', 'agent', 'video']
    if (any(x in frame.f_code.co_name for x in l) or any(x in frame.f_code.co_filename for x in l)):
        print(frame.f_code.co_filename)
        if event == "call":
            indent[0] += 2
            print("-" * indent[0] + "> call function", frame.f_code.co_name)
