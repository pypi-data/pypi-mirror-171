...
def reset_svn_acl(data, project_name):
    ...
    for acl in data:
        base_svn_directory = acl['base_svn_directory']
        ...
        #se
            # if base_svn_directory in new_acl_parser:
            #     new_acl_parser.set(base_svn_directory, user, '')
            #     new_acl_parser.set(base_map_svn_directory, user, '')
            # else:
            #     new_acl_parser[base_svn_directory] = {
            #         '@admin':'rw',
            #         '*':''
            #     }
            #     new_acl_parser[base_map_svn_directory] = {
            #         '@admin':'rw',
            #         '*':''
            #     }
...
class All_Task_File_Access_Control(Resource):
    def post(self, project_name):
        ...
        if config.ENABLE_JOB_QUEUE:
            ...
            reset_svn_acl(data, project_name)
            project_repo_url = os.path.join(SVN_PARENT_URL, project_name)
            return jsonify(message=f'project created', project_name=project_name, svn_url=project_repo_url)
...
api.add_resource(All_Task_File_Access_Control, '/reset_task_acl/<string:project_name>')