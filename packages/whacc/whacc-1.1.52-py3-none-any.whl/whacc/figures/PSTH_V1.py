...
smooth_by = 5
bd = '/Users/phil/Dropbox/HIRES_LAB/WhACC PAPER/figures/misc/PSTH_few_cells_2/'
# utils.clear_dir(bd)
format = 'png'
for ii, cell_num in enumerate(to_loop_cells):
    ...
    plt.xlim(-50, 250)
    plt.legend()
    plt.savefig(bd+os.sep+c['title1'] + '.'+format, transparent=True, format=format)
    plt.close()

    plt.figure()
    plt.imshow(c['template_img'])

    plt.savefig(bd+os.sep+c['title1'] + '_POLE.'+format, transparent=True, format=format)
    plt.close()
