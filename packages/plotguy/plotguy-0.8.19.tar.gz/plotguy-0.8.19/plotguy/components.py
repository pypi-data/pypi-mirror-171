...
class Components:
    ...
    def aggregate_df(self):
        ...
        for folder in folders:
            ...
            for i, row in df_saved.iterrows():
                ...
                dict['performance'] = {
                    'initial_capital': "{:,}".format(int(row['initial'])),
                    'return': "{:.0%}".format(row['return']/100),
                    'mdd': "{:.0%}".format(row['mdd']/100),
    ...
    def aggregate_performance(self, performance_list):
        return_on_capital = "{:.1%}".format(performance_list[0])
        annualized_return = "{:.1%}".format(performance_list[1])
        annualized_sr = round(performance_list[2],2)
        mdd_pct = "{:.1%}".format(performance_list[3])

        div = html.Div([
            dbc.Row([
                dbc.Col('Return on Capital', width=2),
                dbc.Col(return_on_capital, width=2),
            ]),
            dbc.Row([
                dbc.Col('Annualized Return', width=2),
                dbc.Col(annualized_return, width=2),
            ]),
            dbc.Row([
                dbc.Col('Sharp Ratio', width=2),
                dbc.Col(annualized_sr, width=2),
            ]),
            dbc.Row([
                dbc.Col('MDD Percentage', width=2),
                dbc.Col(mdd_pct, width=2),
            ]),
        ])
