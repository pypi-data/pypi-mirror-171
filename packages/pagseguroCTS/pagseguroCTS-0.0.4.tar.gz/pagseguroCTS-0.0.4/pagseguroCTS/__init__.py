...
class PagSeguro(object):
    ...
    def __init__(self, email, token, data=None, payload=None, config=None):
    ...
    def post(self, uri, payload):
        """do a post request"""
        url = self.config.PAYMENT_API_URL + uri
        return requests.post(url, data=payload, headers=self.config.HEADERS)
