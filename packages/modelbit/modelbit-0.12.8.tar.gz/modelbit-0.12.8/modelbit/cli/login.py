...
def loginAndPickWorkspace(mbApi: MbApi) -> Optional[CloneInfo]:
  try:
    ...
    output(f"Failed to reach login servers for {mbApi._cluster}. Please contact support.")
    exit(1)
  ...
  cloneInfo = None
  triesLeft = 150
  while not (cloneInfo := checkAuthentication(mbApi)) and triesLeft > 0:
    triesLeft -= 1
    sleep(3)
  if cloneInfo:
...
def checkAuthentication(api) -> Optional[CloneInfo]:
