...
def test_multi_separators_1():
    ...
    with pytest.raises(AssertionError):
        df = PdReadFlexibleCsv(  # noqa
            path=csv_path, try_separator=",", expected_columns=["series", "start", "end"], parse_dates=None,
        ).df
...
def test_no_error_with_dates():
    ...
    with pytest.raises(AssertionError):
        df = PdReadFlexibleCsv(  # noqa
            path=csv_path, try_separator=",", expected_columns=["series", "start", "end"], parse_dates=["start", "end"],
        ).df
