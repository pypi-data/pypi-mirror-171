...
class JsonLDConverter:
    ...
    ORKG_PREFIX = "https://orkg.org/"
    CLASS_PREFIX = f"{ORKG_PREFIX}class/"
    RESOURCE_PREFIX = f"{ORKG_PREFIX}resource/"
    PREDICATE_PREFIX = f"{ORKG_PREFIX}property/"
    ...
    def __handle_csvw_dataset(self, lol: List[List], context: dict) -> dict:
        ...
        table_node = {'@type': [f'{self.CSVW_PREFIX}Table'], 'label': 'Table'}
        # create columns and assign them
        columns = lol[0]
        context['columns'] = f'{self.CSVW_PREFIX}columns'
        context['name'] = f'{self.CSVW_PREFIX}name'
        context['number'] = f'{self.CSVW_PREFIX}number'
        column_nodes = [{'@type': [f'{self.CSVW_PREFIX}Column'], 'label': c, 'name': c, 'number': i} for i, c in enumerate(columns, start=1)]
        table_node['columns'] = column_nodes
        # iterate over rows and create them
        row_nodes = []
        for i, r in enumerate(lol[1:], start=1):
            row = {'@type': [f'{self.CSVW_PREFIX}Row'], 'titles': f'Row {i}', 'cells': [], 'number': i}
            # run through cells
            for c in r:
                row['cells'].append({'@type': [f'{self.CSVW_PREFIX}Cell'], 'value': c})
            # append row to rows
            row_nodes.append(row)
        context['rows'] = f'{self.CSVW_PREFIX}rows'
        context['cells'] = f'{self.CSVW_PREFIX}cells'
        context['value'] = f'{self.CSVW_PREFIX}value'
        context['titles'] = f'{self.CSVW_PREFIX}titles'
...
if __name__ == '__main__':
    ...
    print(JsonLDConverter().convert_2_jsonld(json.loads(sample)))
