    ...
    for key in conditions.keys():
        ...
        if type(conditionValue) == list:
            if sqlTemplate is None or sqlTemplate not in (sqlTemplateIn,sqlTemplateNotIn):
                conditionArray.append(sqlTemplateIn.format(key,','.join(['%s'] * len(conditionValue))))
            else:
                conditionArray.append(sqlTemplate.format(key,','.join(['%s'] * len(conditionValue))))
            for item in conditionValue:
                params.append(item) 
        else:
            conditionArray.append(sqlTemplate.format(key))
