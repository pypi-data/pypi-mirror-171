...
class TabularPatientBase(Dataset):
    ...
    metadata: {
        ...
        } # see https://docs.sdv.dev/rdt/transformers-glossary/browse-transformers for all avaliable transformers.

        It is highly recommend to provide the metadata of the input tabular data.
        The basic tabular patient data includes five types of features: `categorical`, `numerical`, `boolean`, `datetime`.
    ...
    transform: bool
        Whether or not transform raw self.df by hypertransformer, default=True. If set False, self.df will keep as the same as the passed one.
    ...
    def _parse_metadata(self):
        ...
        if 'transformers' not in metadata:
            # set default transformer for each type of data
            self.ht.detect_initial_config(self.df, verbose=False)
            self.ht.update_sdtypes(metadata['sdtypes'])
        elif 'sdtypes' not in metadata:
            # set custom transformers for columns
            self.ht.detect_initial_config(self.df, verbose=False)
            self.ht.update_transformers(metadata['transformers'])
        else:
            self.ht.set_config(metadata)
        self.ht.fit(self.df)
