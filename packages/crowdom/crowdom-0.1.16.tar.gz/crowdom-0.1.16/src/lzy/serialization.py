...
class Class(ProtobufSerializer[base.Class]):
    ...
    def serialize(obj: base.Class) -> 'Class':
        cls = object_types_registry[type(obj)]
        return Class(value=obj.value, cls=cls)

    def deserialize(self) -> base.Class:
        cls = object_types_registry_reversed[self.cls]
...
class If(ProtobufSerializer[base.If]):
    ...
    then_if: Optional['If'] = field(2)
    then_consequence: Optional[Consequence] = field(3)
    else_if: Optional['If'] = field(4)
    else_consequence: Optional[Consequence] = field(5)
    ...
    def serialize(obj: base.If) -> 'If':
        return If(
            ...
            then_if=If.serialize(obj.then)
            if obj.then and isinstance(obj.then, base.If)
            else None,
            then_consequence=Consequence.serialize(obj.then)
            if obj.then and isinstance(obj.then, base.Consequence)
            else None,
            else_if=If.serialize(obj.else_)
            if obj.else_ and isinstance(obj.else_, base.If)
            else None,
...
class LabelsDisplayType(StrEnumSerializer[base.LabelsDisplayType]):
    @staticmethod
...
class ClassMeta(ObjectMeta, ProtobufSerializer[base.ClassMeta]):
    available_labels: Optional[If] = field(100)
    input_display_type: LabelsDisplayType = field(101)
    text_format: TextFormat = field(102)
...
class TextMeta(ObjectMeta, ProtobufSerializer[objects.TextMeta]):
    format: TextFormat = field(100)
    validation: Optional[TextValidation] = field(101)
