...
def deploy_model(
    ...
    try:
        ...
        logging.info("Setting up ephemeral virtuel environment")
        venv_name = "mlops-venv"
        venv_dir = helpers.set_up_requirements(c, install=True, venv_name=venv_name)
        ...
        if c.platform == "local" and c.test_file:
            ...
            try:
                subprocess.check_output(test_command, encoding="UTF-8")
        ...
        # Delete virtual environment setup for testing
        helpers.delete_venv(venv_name=venv_name)
        return "OK"
    except (
        TestError,
        DeploymentError,
