...
def generate_config():
    ...
    role = input("Please enter role (ACCOUNTADMIN):") or "ACCOUNTADMIN"
    warehouse = input("Please enter warehouse (CLEAN_ROOM_SETUP_WH):") or "CLEAN_ROOM_SETUP_WH"
    user = input("Please enter user (clean_room_user):") or "clean_room_user"
    sso = input("do you use SSO for login  (Y)?") or "Y"

    if sso != "Y" and sso != "y":
        password = getpass.getpass("Please enter password:")

    f = open("habu_snowflake_config.yaml", "w")
...
warehouse: {warehouse}
user: {user}
'''.format(accountName=account_name, organizationName=organization_name, role=role, warehouse=warehouse, user=user))
