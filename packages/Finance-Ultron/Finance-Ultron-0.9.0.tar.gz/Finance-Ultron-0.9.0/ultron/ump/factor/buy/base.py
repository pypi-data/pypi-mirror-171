...
from ultron.ump.core.deprecated import Deprecated
from ultron.ump.core.fixes import six
from ultron.ump.core.base import ParamBase
from ultron.ump.slippage.buy_mean import SlippageBuyMean
from ultron.ump.trade.order import Order
from ultron.ump.ump.manager import Manager
from ultron.ump.trade.ml_feature import MlFeature
from ultron.ump.beta import position_base
from ultron.ump.beta.atr_position import AtrPosition
...
class FactorBuyBase(six.with_metaclass(ABCMeta, ParamBase)):
    ...
    def __init__(self, capital, kl_pd, combine_kl_pd, benchmark, **kwargs):
        ...
        self.ump_manger = Manager(factor=self)
    ...
    def _position_class_init(self, **kwargs):
        """仓位管理类构建"""
        # 仓位管理，默认AtrPosition
        if position_base.g_default_pos_class is None:
            self.position_class = AtrPosition
            # 仓位管理类构建关键字参数默认空字典
            self.position_kwargs = dict()
        else:
            # 否则设置了全局仓位管理字典对象，弹出class
            default_pos_class = copy.deepcopy(
                position_base.g_default_pos_class)
        ...
        if 'position' in kwargs:
            ...
            if isinstance(position, six.class_types):
                ...
                raise TypeError(
                    '_position_class_init position type is {}'.format(
                        type(position)))
    ...
    def __str__(self):
        ...
        return '{}: slippage:{}, position:{} \nkl:\n{}'.format(
            self.__class__.__name__, self.slippage_class, self.position_class,
            self.kl_pd.info())
    ...
    def make_buy_order_ml_feature(self, day_ind):
        ...
        :return:
        """
        return MlFeature().make_feature_dict(self.kl_pd,
                                             self.combine_kl_pd,
                                             day_ind,
                                             buy_feature=True)
    ...
    def past_today_kl(self, today, past_day_cnt):
        ...
        """
        end_ind = self.combine_kl_pd[self.combine_kl_pd.date ==
                                     today.date].key.values[0]
    ...
    @Deprecated(
        'kelly object now use dict to build, it will be remove next version!')
...
class FactorBuyXD(FactorBuyBase):
    ...
    def read_fit_day(self, today):
        ...
        self.xd_kl = self.kl_pd[self.today_ind - self.xd + 1:self.today_ind +
                                1]
