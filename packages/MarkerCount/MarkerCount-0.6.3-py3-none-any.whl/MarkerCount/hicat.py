...
def my_polyfit2( x, y ):
    ...
    M = np.array([x**2, x, np.ones(len(x))])
    Rxx = M.dot(M.transpose())/len(x) + np.eye(3)*1e-6
    rxy = (np.array([y,y,y])*M).mean(axis = 1)
    z = np.linalg.solve(Rxx, rxy)
...
def select_variable_genes(log1p_X, N_genes = 2000):
    ...
    ## Fit
    m_sel = m # Xs.mean(axis = 0)
    s_sel = s # Xs.std(axis = 0)

    lm_sel = np.log10(m_sel + 0.000001)
    ls_sel = np.log10(s_sel + 0.000001)
    
    z = np.polyfit( lm_sel, ls_sel, 2 )
    # z = my_polyfit2( lm_sel, ls_sel )
    p = np.poly1d(z)
