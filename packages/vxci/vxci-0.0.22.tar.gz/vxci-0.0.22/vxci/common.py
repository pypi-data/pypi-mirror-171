...
def test_module(config):
    ...
    for instance_type in INSTANCE_TYPES:
        ...
        queries = deactivate2queries(instance_type)
        for query in queries:
            cmd_query = ["/bin/sh", "-c", 'psql test -XtAc "%s" && echo "pass"' % query]
            query_res = exec_cmd(config['container_name'], cmd_query, 'odoo')
            query_res_lines = query_res.splitlines()
            if "pass" in query_res_lines[-1:]:
                continue
            _logger.error('query error "%s" Result: "%s"', query, query_res_lines[:-1])
            success = False
    return success
