...
def get_gulp_force_from_drv(fdrv,wdftb=False):
    forces = []
    ...
    # print(forces)
    if wdftb:
       with open('results.tag','w') as fdftb:
            print('forces              :real:2:',file=fdftb)
            for f in forces:
               print(f[0],f[1],f[2],file=fdftb)
    return np.array(forces)

def get_gulp_forces(images,traj='gulp_force.traj',ffield='reaxff_nn',wdftb=False):
    ...
    for atoms in images:
        write_gulp_in(atoms,runword='gradient nosymmetry conv qite verb',
                      ...
                      lib=ffield)
        system('gulp<inp-gulp>out')
        forces = get_gulp_force_from_drv('gulp.drv',wdftb=wdftb)
        #atoms.forces = forces
