...
def init_framework(sf_connection, organization_id: str, habu_org_name_account_name_combo: str, warehouse: str):
    ...
    if habu_org_name_account_name_combo is None:
        result = snowflake_utils.fetch_one_query(sf_connection, "CALL HABU_CLEAN_ROOM_COMMON.HABU_SCHEMA.INIT_FRAMEWORK('%s')" % (organization_id))
    else:
        result = snowflake_utils.fetch_one_query(sf_connection, "CALL HABU_CLEAN_ROOM_COMMON.HABU_SCHEMA.INIT_FRAMEWORK('%s', '%s')" % (organization_id, habu_org_name_account_name_combo))
