...
import json
from concurrent.futures import ThreadPoolExecutor, Future  # pylint: disable=W0611
import copy
import os
import time
import threading
...
class BufferedHandleMultiple(MemoryHandler):
    ...
    def __init__(  # pylint: disable=R0913
        ...
        self._interval_thread = threading.Thread(target=self._flush_thread_handler)
...
class HTTPMultiRecordHandler(MultiRecordHandlerBase):
    ...
    def __init__(
        ...
        """
        super().__init__()
        self._excecutor = ThreadPoolExecutor(thread_name_prefix="HTTPMultiRecordHandler")
        if not target_uri:
    ...
    def close(self) -> None:
        ...
        try:
            ...
            self.flush()
            # Let tasks finish
            self._excecutor.shutdown(wait=True)
        finally:
            super().close()
    ...
    def emit_multiple(self, records: MultiRecordType) -> None:
        ...
        def do_post(records: MultiRecordType) -> None:  # pylint: disable=R0914
            ...
            for record in records:
                ...
                extra = {key: getattr(record, key) for key in extra_keys}
                record.extra = cast(Mapping[str, Any], extra)  # type:ignore
                if not extra:
                    records_by_labels[0].append(record)
                    continue
                labels_key = hash(FrozenOrderedDict(extra))
            ...
            for bkey in sorted(records_by_labels.keys()):
                ...
                data = "\n".join(formatted)
                if extra:
                    extra_headers.update({"labels_json": json.dumps(extra)})
                try:
        ...
        try:
            fut = self._excecutor.submit(do_post, records)
