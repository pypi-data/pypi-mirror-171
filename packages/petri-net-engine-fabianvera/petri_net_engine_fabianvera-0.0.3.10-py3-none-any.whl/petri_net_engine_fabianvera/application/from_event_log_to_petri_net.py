    ...
    def execute(self, url_dataframe):
        ...
        for key, item in cases:
            ...
            for i in range(len(case)):
                ...
                is_input_valid = True
                for petri_net in petri_net.entries:
                    is_input_valid = is_input_valid and (petri_net['place'] != str(case.iloc[i]['place_id']) or \
                                     petri_net['transition'] != str(case.iloc[i]['transition_id']))
                if is_input_valid:
                    petri_net.add_input(str(case.iloc[i]['place_id']), str(case.iloc[i]['transition_id']), 1)
                    if i + 2 > len(case):
                        break
                    # input = {'place': str(case.iloc[i + 1]['place_id']), 'transition': str(case.iloc[i + 1]['transition_id']),
                    #          'number': 1}
                    # output = {'place': str(case.iloc[i + 1]['place_id']),
                    #          'transition': str(case.iloc[i + 1]['transition_id']),
                    #          'number': 1}
