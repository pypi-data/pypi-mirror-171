...
class NewsEncoder(torch.nn.Module):
    def __init__(
        ...
        assert len(dataset_attributes["news"]) > 0
        ...
        self.text_encoders = nn.ModuleDict(
            {
                ...
                for name in (
                    set(dataset_attributes["news"]) & set(text_encoders_candidates)
                )
            }
        )
        ...
        self.element_encoders = nn.ModuleDict(
            {
                ...
                for name in (
                    set(dataset_attributes["news"]) & set(element_encoders_candidates)
                )
            }
        )
        if len(dataset_attributes["news"]) > 1:
    ...
    def forward(self, news):
        ...
        element_vectors = [
            encoder(news[name]) for name, encoder in self.element_encoders.items()
        ]
