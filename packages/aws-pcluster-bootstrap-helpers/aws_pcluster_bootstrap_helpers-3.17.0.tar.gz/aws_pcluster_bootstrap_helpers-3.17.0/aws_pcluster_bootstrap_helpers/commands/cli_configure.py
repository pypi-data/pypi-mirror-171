...
def configure(region: str = "us-east-1"):
    ...
    vpc_choice = list(
        filter(lambda x: x["label"] == answers["vpc_id"], vpc_choices_options)
    )[0]
    ...
    answers = inquirer.prompt(questions)
    head_subnet = list(
        filter(lambda x: x["label"] == answers["head_subnet"], subnet_choices_options)
    )[0]
    compute_subnet = list(
        filter(
            lambda x: x["label"] == answers["compute_subnet"], subnet_choices_options
        )
    )[0]
    ...
    network_data = dict(vpc_id=vpc_id, subnet_ids=subnet_ids)
    print(network_data)
    return network_data
