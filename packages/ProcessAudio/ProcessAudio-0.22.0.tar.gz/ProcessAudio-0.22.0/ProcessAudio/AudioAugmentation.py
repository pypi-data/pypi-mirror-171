    ...
    def middleware_in(atributo1: str = "", ):
        def _middleware_in(f):
            def wrapper(self, *args, **kwargs):
                ...
                print("\t"*2,  f"Middleware in {f.__name__} ({atributo1})", end="")
                ...
                if ejecucion_ok:
                    ...
                    if self.save is not None:
                        try:
                            ...
                            print(f" - Save in {self.save + self.name_file + '_' + f'{f.__name__}.wav'}", end="")
                        except Exception as e:
                            print(f" - Error to save file ({self.save + self.name_file + '_' + f'{f.__name__}.wav'}): {e}", end="")
                            print(type(salida))
...
if __name__ == "__main__":
    ...
    muestras_wav, instrumentos = Audio_K(data_audio, label=label_audio, save='data/output/').build_all()

    processAudio = ProcessAudio(rate)
    for id_audio, dat in enumerate(muestras_wav):
        processAudio.set_data(dat)
        caracteristicas = processAudio.build_all(id_audio)  # Extrayendo caracteristicas audios, salen 26 caracteristicas
