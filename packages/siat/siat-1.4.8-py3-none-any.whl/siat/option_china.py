...
def hs300option_compare(option1,option2,loc1='best',loc2='best'):
    ...
    #获取期权价格
    df1=hs300option_price(maturity=maturity1,exercise=exercise1,direction=direction1,graph=False)
    df2=hs300option_price(maturity=maturity2,exercise=exercise2,direction=direction2,graph=False)
    
    #内连接：取共同日期期间
    import pandas as pd
    df12=pd.merge(df1,df2,how="inner",left_index=True,right_index=True)

    ticker1="期权1"; colname1='SO Close_x'; label1="收盘价"
    ticker2="期权2"; colname2='SO Close_y'; label2="收盘价"
    ylabeltxt="收盘价"
    titletxt='沪深300股指期权：影响因素与价格趋势对比'
...
def fin_option_risk_sse(option,maturity,exercise,date,printout=True):
    """
    功能：显示指定上交所金融期权的风险指标
    option: 期权名称
    maturity: 到期年月YYMM
    ...
    if not check_date(date):
        print("  #Error(fin_option_risk_sse): invalid date",date)
        return None
    akdate=convert_date_ts(date)
    ...
    try:
        ...
        print("  #Warning(fin_option_risk_sse): no data available for this date",date)
        return None
...
def fin_option_maturity_risk_sse(option,exercise,date,measure='delta',graph=True,loc1='best',loc2='best'):
    """
    ...
    #初步检查：交易日期
    if not check_date(date):
        print("  #Error(fin_option_maturity_risk_sse): invalid date",date)
        return None
    akdate=convert_date_ts(date)
    ...
    try:
        ...
        print("  #Warning(fin_option_maturity_risk_sse): no data available for this date",date)
        return None
    ...
    footnote1="期权："+option+"，行权点位："+str(exercise)+"，交易日期："+date
    import datetime; today = datetime.date.today()
...
if __name__=='__main__':     
    df=fin_option_maturity_risk_sse(option='300ETF',exercise=4500,date='2022-9-26',measure='implied volatility')
...
def fin_option_exercise_risk_sse(option,maturity,date,measure='delta',graph=True,loc1='best',loc2='best'):
    """
    ...
    #初步检查：日期
    if not check_date(date):
        print("  #Error(fin_option_exercise_risk_sse): invalid date",date)
        return None
    akdate=convert_date_ts(date)
    ...
    try:
        ...
        print("  #Warning(fin_option_exercise_risk_sse): no data available for this date",date)
        return None
    ...
    footnote1="行权点位（指数点位）"
    footnote2="期权："+option+"，到期时间："+maturity+"，交易日期："+date
    import datetime; today = datetime.date.today()
...
if __name__=='__main__':     
    df=fin_option_exercise_risk_sse(option='300ETF',maturity='2209',date='2022-9-26',measure='implied volatility')
...
def fin_option_time_risk_sse(option,maturity,exercise,start,end,measure='delta', \
                             graph=True,loc1='best',loc2='best',date_range=False,date_freq=False):
    """
    ...
    while curdatepd <= endpd:
        #print("  Retrieving data for",curdate,"......")
