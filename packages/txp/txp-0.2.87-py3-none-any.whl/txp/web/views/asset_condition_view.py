...
class AssetConditionsView(MainView):
    ...
    def get_data(self):
        ...
        for asset in self.data["machines"].keys():
            if asset != "Showroom_Fridge": continue
        ...
        for asset in self.data["machines"].keys():
            ...
            if self.states[asset]:
                asset_events = txp.common.utils.bigquery_utils.get_events_for_state(
                    self._tenant_id,
                    self._events_dataset,
                    asset,
                    list(self.states[asset]['events']),
                    self._days_report,
                    self.bqclient
                )
            else:
                asset_events = None
            self.events.update({asset: asset_events})
