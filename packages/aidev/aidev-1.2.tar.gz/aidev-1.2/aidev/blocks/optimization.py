...
from aidev.utilities.types import NewOptimization
...
class NelderMeadSearch(NewOptimization):
    def __init__(
...
class GeneticAlgorithm(NewOptimization):
    def __init__(
        self, objective: Callable[..., List[float]], features, popsize
    ) -> None:
        """
...
class ParticleSwarm(NewOptimization):
    def __init__(
        self, objective: Callable[..., List[float]], features, popsize
    ) -> None:
        """
...
class NSGA_III(NewOptimization):
    def __init__(
        self, objective: Callable[..., List[float]], features, popsize
    ) -> None:
        """
    ...
    def _algorithm(self):
        ...
        """
        n_ref = len([name for name in self.features.weights.values()])
        ref_dirs = get_reference_directions("energy", n_ref, n_points=10, seed=1)
        algorithm = NSGA3(
