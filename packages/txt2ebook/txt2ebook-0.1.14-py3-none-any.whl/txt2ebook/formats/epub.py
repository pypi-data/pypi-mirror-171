...
class EpubWriter:
    ...
    def write(self) -> None:
        ...
        if self.book.volumes:
            logger.debug("Generate %s EPUB volumes", len(self.book.volumes))
            ...
            for volume in self.book.volumes:
                ...
                if self.volume_page:
                    logger.debug("Create separate volume page: %s", volume)
                    book.toc.append((html_volume, html_chapters))
                else:
                    book.toc.append(
                        (epub.Section(volume.title), html_chapters)
                    )
        else:
            logger.debug("Generate %s EPUB chapters", len(self.book.chapters))
