...
@LastEditTime: 2022-10-12 19:12:38
...
class FrameBaseHandler(BaseApiHandler):
    ...
    def create_operation_log(self, operation_type=1, model_name="", handler_name="", old_detail=None, update_detail=None, operate_user_id="", operate_user_name="", operate_role_id="", title="", operation_desc="", operation_ways=1):
        """
        :description: 创建操作日志
        :param operation_type：操作类型（还原和复制不在使用，统一到操作）：1-增加，2-编辑，3-删除，4-还原，5-复制,6-导出,7-操作
        ...
        operation_log.operation_ways = operation_ways
        ...
        if plat_type == 1 and not operate_user_id:
            operation_log.operate_user_id = self.get_open_id()
            operation_log.operate_user_name = self.get_user_nick()
