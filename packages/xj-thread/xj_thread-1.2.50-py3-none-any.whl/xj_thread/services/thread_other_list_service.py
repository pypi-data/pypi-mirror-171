...
class ThreadOtherListServices:
    ...
    @staticmethod
    def thread_category(category_value=None, category_id=None):
        """
        类别。类似于版块大类的概念，用于圈定信息内容所属的主要类别
        """
        thread_category_obj = ThreadCategory.objects.annotate(category_value=F('value')).values(
            "id",
            "platform_code",
            "category_value",
            "name",
        ...
        category_set = ThreadCategory.objects.values()
        if category_value:
            category_set = category_set.filter(value=category_value)
        if category_id:
