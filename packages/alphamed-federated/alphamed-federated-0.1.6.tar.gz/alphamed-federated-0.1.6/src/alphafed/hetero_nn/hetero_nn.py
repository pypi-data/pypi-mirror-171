...
from .contractor import (CheckinEvent, CollaboraterCompleteEvent, HeteroNNContractor, SendFeatureEvent,
                         SendNoisedPorjectionEvent, SendNoisedWGradEvent,
...
class HeteroNNScheduler(Scheduler, metaclass=ABCMeta):
    ...
    def make_feature_model(self) -> nn.Module:
        ...
        str_keyword is used by the host to distinguish features from collaboraters
    ...
    def train_ids(self) -> Set[str]:
        ...
        if not hasattr(self, '_train_ids'):
            ...
            self._train_ids, self._test_ids = self.split_data_set(self.id_intersection.copy())
...
class HeteroNNHostScheduler(HeteroNNScheduler):
    ...
    def _distribute_cipher_projection(self):
        """Distribute cipher feature projection to collaboraters."""
        self.push_log('Distributing cipher projection to collaboraters ...')
        ...
        for _partner, _cipher_feature in self._alpha_map.items():
            ...
            self.push_log(f'Distributed cipher projection to collaborater ID: {_partner}')

    @torch.no_grad()
    def _collect_noised_projection(self):
        """Collect noised but plain projection from collaboraters."""
        self.push_log('Collecting noised but plain projection from collaboraters ...')
    ...
    def _distribute_cipher_w_grad(self):
        self._epsilon_host = random.randint(1, 2**10)
        self.push_log('Distributing cipher W grad to collaboraters ...')
        ...
        self.push_log('Distributed all cipher W grad to collaboraters.')

    def _collect_noised_w_grad(self):
        """Collect decrypted but noised W grad of project layer from collaboraters."""
        self.contractor.notify_ready_for_noised_w_grad(round=self._round)
        self.push_log('Collecting noised W grad from collaboraters ...')
    ...
    def _distribute_cipher_feature_grad(self):
        """Send cipher feature grad to collaboraters."""
        self.push_log('Distributing cipher feature grad to collaboraters ...')
        for _partner, feature_dict in self._alpha_map.items():
            if _partner == self.id:
                continue
        ...
        self.push_log('Distributed all cipher feature grad to collaboraters.')
    ...
    def _save_model(self):
        ...
        with open(f'{os.path.join(self._save_path, "feature_model.pt")}', 'wb') as f:
            torch.save(self.feature_model.state_dict(), f)
        with open(f'{os.path.join(self._save_path, "project_layer.pt")}', 'wb') as f:
            torch.save(self.project_layer.state_dict(), f)
        with open(f'{os.path.join(self._save_path, "infer_model.pt")}', 'wb') as f:
    ...
    def _prepare_task_output(self) -> Tuple[str, str]:
        ...
        feature_model_file = os.path.join(self._save_path,
                                          f'scripted_feature_{self.feature_key}.pt')
        with open(feature_model_file, 'wb') as f:
            torch.save(self.feature_model.state_dict(), f)
        project_layer_file = os.path.join(self._save_path, 'scripted_project_layer.pt')
        with open(project_layer_file, 'wb') as f:
            torch.save(self.project_layer.state_dict(), f)
        infer_model_file = f'{os.path.join(self._save_path, "scripted_infer.pt")}'
    ...
    def _wait_for_all_complete(self):
        """Wait for all collaboraters complete their tasks."""
        self.push_log('Waiting for all collaboraters complete their tasks ...')
        results = {_peer_id: False for _peer_id in self._partners}
        for _event in self.contractor.contract_events():
            if isinstance(_event, CollaboraterCompleteEvent):
                results[_event.peer_id] = True
                if all(results.values()):
                    break
        self.push_log('All collaboraters have completed their tasks.')
