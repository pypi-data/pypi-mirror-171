...
def merge_parts(pattern, output_name="", sep=","):
    data = []
    column_names = []
    for idx, csvfile in enumerate(glob.glob(pattern)):
        print(csvfile)
        df = pd.read_csv(csvfile, encoding='utf-8', sep=sep, dtype=str)
        column_names = df.columns.tolist()
        data.append(df)

    output_csv_path = output_name
    data = pd.concat(data)
    if len(output_name) > 0:
        print("Saving ", output_csv_path)
...
def read_scp(scp, sep=" ", clean=False):
    ...
    for i in data:
        ...
        text = "".join(text)
        if clean:
            text = text.replace(".", "").replace("，", "").replace("<UNK>", "").replace("。", "").replace("、", "").replace(" ", "").replace(",", "")
