...
class BaseDataSource(Fco):
    ...
    def get_dataframe(
        ...
        *,
        ...
        if isinstance(self.ds_spec.batch_source, specs.SparkBatchSourceSpec):
            if not self.ds_spec.batch_source.supports_time_filtering and (start_time or end_time):
                raise errors.DS_INCORRECT_SUPPORTS_TIME_FILTERING

            if conf.get_bool("QUERYTREE_ENABLED"):
                node = build_datasource_scan_node(
                    self.ds_spec, for_stream=False, start_time=start_time, end_time=end_time
                )
                return TectonDataFrame._create(node)
            else:
                df = data_source_helper.get_ds_dataframe(
                    spark,
                    data_source=self.ds_spec,
        ...
        if apply_translator:
            timestamp_key = self.ds_spec.batch_source.timestamp_field
            ...
            if conf.get_bool("QUERYTREE_ENABLED"):
                node = RawDataSourceScanNode(self.ds_spec).as_ref()
                return TectonDataFrame._create(node)
            else:
                df = data_source_helper.get_non_dsf_raw_dataframe(spark, self.ds_spec.batch_source)
                return TectonDataFrame._create(df)
