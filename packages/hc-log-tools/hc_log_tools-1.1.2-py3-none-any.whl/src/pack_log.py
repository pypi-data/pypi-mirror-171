...
def init_input_args(sub_parser):
    ...
    sub_parser.add_argument(
        "--recursive", action="store_true", help="是否递归查找子目录, 默认为True", default=True
    )
    return sub_parser
...
def pack_log(input_args):
    ...
    # terminal process bar for compress with alive-progress with IO info
    with alive_bar(len(log_files)) as bar:
    ...
    print(f"compress complete, total {len(log_files)} files, output file: {input_args.output_file}")
