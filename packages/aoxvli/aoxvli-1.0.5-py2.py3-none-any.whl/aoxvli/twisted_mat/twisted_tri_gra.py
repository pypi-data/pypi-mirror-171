...
class ContiAbdTtgInst(ContiTtgInst):
    ...
    base_intra_coup = PubMeth.list2mat([2, 2591, 2591, 2]) #* 0.95

    res_intra_coup = PubMeth.list2mat([-13, 2608, 2608, -13]) #* 0.95

    twist_intra_coup = PubMeth.list2mat([11, 2606, 2606, 11]) #* 0.95
...
class TightTtgInst(TightTbgInst):
    ...
    def plot_atoms(self, save_or_not=True, fig_format='.svg', super_cell_view=True):
        ...
        ax = plt.gca()
        ax.set_aspect('equal')
        plt.title(r"Atomic Structure of Twisted Trilayer Graphene $\theta=%.3f \degree$" % self.twist_angle, fontsize=14)
    ...
    def inter_term(self, k_arr, distance_arr, layer1_i, layer2_i):
        ...
        return (self.t_intra * exp(- (d_norm - self.a0_const / sqrt(3)) / self.delta_0_par) * (1 - value_cos ** 2) + self.t_inter *
