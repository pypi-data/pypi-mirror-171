...
class CommandHandler:
    ...
    def check_update(self, update: WhatsAppUpdate):
        if update.get_message()._type == "text" and update.get_message().text.strip("/").lower() == self.command.lower():
            return True
...
class RegexFilter:
    ...
    def __call__(self, update: WhatsAppUpdate) -> bool:
        if update.get_message()._type == "text":
            return re.search(self.regex, update.get_message().text)
        else:
            return False
    ...
    def __call__(self, update: WhatsAppUpdate):
        ...
        try:
            ...
            response = req.post(config["rasa"]["url"] + "/api/projects/default/logs", params={
                                "q": update.get_message().text}, headers={'Authorization': 'Bearer {}'.format(RASA_TOKEN)},  timeout=(3, 8))
            print(response.json())
