...
class MPNN(ReaxFF):
  ...
  def get_bondorder(self,t,bd,atomi,atomj):
      ...
      if self.MessageFunction==1:
         ...
         self.Dbi[bd]  = Di-h
         self.Dbj[bd]  = Dj-h
         Fi   = self.fmessage(flabel,b[0],self.nbd[bd],[self.Dbi[bd],self.Dbj[bd],self.Hsi[t-1][bd],self.Hpi[t-1][bd],self.Hpp[t-1][bd]],
                              layer=self.mf_layer[1])
         Fj   = self.fmessage(flabel,b[1],self.nbd[bd],[self.Dbj[bd],self.Dbi[bd],self.Hsi[t-1][bd],self.Hpi[t-1][bd],self.Hpp[t-1][bd]],
                              layer=self.mf_layer[1])
         F    = Fi*Fj
         Fsi,Fpi,Fpp = tf.unstack(F,axis=2)
         ...
         Fi   = self.fmessage(flabel,b[0],self.nbd[bd],[self.Dbi[bd],h,self.Dbj[bd]],layer=self.mf_layer[1])
         Fj   = self.fmessage(flabel,b[1],self.nbd[bd],[self.Dbj[bd],h,self.Dbi[bd]],layer=self.mf_layer[1])
         F    = Fi*Fj
         Fsi,Fpi,Fpp = tf.unstack(F,axis=2)
         ...
         Fi   = self.f_nn(flabel,b[0],self.nbd[bd],[self.Dbi[bd],h,self.Dbj[bd]],layer=self.mf_layer[1])
         Fj   = self.f_nn(flabel,b[1],self.nbd[bd],[self.Dbj[bd],h,self.Dbi[bd]],layer=self.mf_layer[1])
                ...
                if self.MessageFunction>1:
                   ...
                   if (bd in self.beup) or (bdr in self.beup):
                      ...
                      for beup_ in self.beup[bd_]:
                          ...
                          fu = tf.where(tf.logical_and(tf.less_equal(self.rbd[bd],r_),
                                             tf.logical_and(
                   ...
                   if (bd in self.belo) or (bdr in self.belo):
                      ...
                      for belo_ in self.belo[bd_]:
                          ...
                          fl = tf.where( tf.logical_and(tf.greater_equal(self.rbd[bd],r_),
                                             tf.logical_and(
                                                 tf.logical_and(tf.less_equal(self.Dbi[bd],Di_u),
                                                                tf.greater_equal(self.Dbi[bd],Di_l)),
                                                 tf.logical_and(tf.less_equal(self.Dbj[bd],Dj_u),
