...
from spacesense.collections.models import Sentinel1SearchResult, Sentinel2SearchResult, Weather, WeatherSearchResult
...
class Client:
    ...
    def __init__(self, id=None, backend_url=None):
        ...
        grpc_client = GrpcClient(self.api_key, self.backend_url)
    ...
    def compute_s1_ard(
        ...
        responses = self.core_stub.GetS1ARD(
            ...
            for response in responses:
                ...
                if response.status == "success":
                    if self.save_to_local:
                        ...
                        result = Sentinel1ResultItem(response.date, response.status, response.data, file_path=file_path)
                    else:
    ...
    def compute_s1_ard_from_search_result(
        ...
        search_results_struct.update(search_result.grpc_message)
    ...
    def fuse_custom(
        ...
        with grpc.secure_channel(
            ...
            stub = backend_pb2_grpc.CoreStub(channel)
    ...
    def weather_search(self, aoi, start_date, end_date, variables: List[str] = None) -> WeatherSearchResult:
        """
                ...
                The following weather parameter are usable in the variables parameter:
                    ...
                    - "VWIs2_seaND": 10m_v_component_of_wind
                ...
                .. note:: weather_search by default returns average 2m temperature an total precipitation only.

                Example
                ----------
                    >>> weather_variables = ["laihigh", "prec", "mintemp", "maxtemp"]
                    >>> weather_df = client.weather_search(aoi = aoi,
                                                    start_date = start_date,
                                                    end_date = end_date,
                                                    variables = weather_variables)

                For more examples, please see the `Fusion with Weather <../notebooks/2_search_s1_s2.html>`_
        ...
        if variables is not None:
            ...
            for var in variables:
                if var.upper() not in Weather.params:
                    unknown_var.append(var)
            if unknown_var:
                raise ValueError(f"Not recognised as variable name: {unknown_var}")
            else:
                variables_struct.update({"variables": variables})
        else:
            variables_struct.update({"variables": ["avgtemp", "prec"]})
