...
class UpperGuideBearingOilTankMetalContentOverLimitDiagnosis(MultipleThresholds_GreaterEqualTo_LesserThan):
    def __init__(self, *args, **kwargs):
        """
        上导轴承油槽金属颗粒含量超限判断

        :math:`T < 950, 上导轴承油槽金属颗粒含量不超限`

        :math:`T ≥ 950, 上导轴承油槽金属颗粒含量超限报警`


        [1] 参数
        ----------
        thresholds:
            list[float],报警限,默认[950]
        truthTable:
            dict,报警真值表,形如:{
            "10": {"dfem_evidence": "[上导轴承油槽水分含量]正常", "dfem_code": ""},
            "01": {"dfem_evidence": "[上导轴承油槽水分含量]实测值(measured)超限报警,阈值threshold", "dfem_code": "SP000470"},
        }
