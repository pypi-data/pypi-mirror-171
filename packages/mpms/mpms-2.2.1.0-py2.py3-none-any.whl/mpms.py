...
class MPMS(object):
    ...
    def __init__(
            ...
            completely_restart_if_subproc_died_unexpectedly=False,
    ):
        ...
        self.collector_thread = None  # type:threading.Thread|None
        ...
        self.completely_restart_if_subproc_died_unexpectedly = completely_restart_if_subproc_died_unexpectedly
    ...
    def _subproc_check(self):
        ...
        for name, p in tuple(self.worker_processes_pool.items()):  # type:str, multiprocessing.Process
            ...
            if bad_exit is not None:
                if self.completely_restart_if_subproc_died_unexpectedly:
                    logger.warning('mpms subprocess %s dies unexpected, exitcode:%s, restarting completely', name, bad_exit)
                    self._completely_restart()
                    break
            else:
                logger.info('mpms subprocess %s stopped normally(by lifecycle), starting new', p)
    ...
    def _completely_restart(self):
        # do internal fake close
        self.close()
        self.join(max_wait_to_kill=60)

        self.worker_processes_pool.clear()
        self.task_queue_closed = False
    ...
    def join(self, close=True, max_wait_to_kill=None):
        """
        ...
        while self.worker_processes_pool:
            for name, p in list(self.worker_processes_pool.items()):  # type: multiprocessing.Process
                if p.is_alive():
                    while True:
                        try:
                            # logger.debug('waiting for %s to exit', p)
                            p.join(timeout=5)
                        except TimeoutError:
                            pass
                        if not p.is_alive():
                            logger.warning("mpms subprocess %s dies", p)
                            break
                        if max_wait_to_kill is not None and time.time() - _start > max_wait_to_kill:
                            logger.warning("mpms subprocess %s timeout, killing", p)
                            p.terminate()
                            time.sleep(3)
                            try:
                                p.close()
                            except Exception as e:
                                logger.warning("mpms error at closing subprocess %s %s", name, repr(e))
                            break
                    logger.debug("mpms subprocess %s closed", p)
                else:
                    logger.warning("mpms subprocess %s dies", p)
                p.close()
                del self.worker_processes_pool[name]
