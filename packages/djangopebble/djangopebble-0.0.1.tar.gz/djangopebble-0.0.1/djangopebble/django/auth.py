...
from rest_framework.authentication import BaseAuthentication
from rest_framework.exceptions import AuthenticationFailed
from rest_framework_jwt.serializers import jwt_decode_handler, jwt_encode_handler

from user.models import User


class CustomizeJWTAuthentication(BaseAuthentication):

    # 该参数用于控制是否启用利用redis实现用户退出后token自动失效的功能
    USE_REDIS_JUDGE_LOGOUT = True
    ...
    def authenticate(self, request):
        ...
        if token:
            ...
            try:
                ...
                if self.USE_REDIS_JUDGE_LOGOUT:
                    from django_redis import get_redis_connection
                    import time
                    conn = get_redis_connection()
                    # 只判断token中不一样的部分
                    token_sign = token.split('.')[2]
                    if conn.sismember(user_id, token_sign):
                        # 在黑名单里说明是用户手动退出的token
                        msg = 'token过期'
...
class AnonymousCustomizeJWTAuthentication(BaseAuthentication):
