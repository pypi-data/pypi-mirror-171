...
import pandas as pd
import re
import cosima_core.communication_scenario as communication_scenario
...
if __name__ == "__main__":
    ...
    for index, row in df.iterrows():
        ...
        if len(infrastructure_changes) > 0:
            communication_scenario.main(n_agents, network, parallel, offset, row["until"], infrastructure_changes)
        else:
            communication_scenario.main(n_agents, network, parallel, offset, row["until"])
        time.sleep(5)
