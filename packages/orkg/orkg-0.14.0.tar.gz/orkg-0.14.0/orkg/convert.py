...
class JsonLDConverter:
    ...
    def __handle_csvw_dataset(self, lol: List[List], context: dict) -> dict:
        ...
        column_nodes = [{'@type': [f'{self.CLASS_PREFIX}Column'], 'label': c, 'name': c, 'number': i} for i, c in enumerate(columns, start=1)]
        ...
        for i, r in enumerate(lol[1:], start=1):
            ...
            # run through cells
            for c in r:
                row['cells'].append({'@type': [f'{self.CLASS_PREFIX}Cell'], 'value': str(c) if c is not np.nan else None})
            # append row to rows
