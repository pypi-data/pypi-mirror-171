...
# 获取某个字段的类型，获取外键及多对多关系对应的模型
# 参考 https://www.codenong.com/10347210/
User = get_user_model()
meta = User._meta
field_names = [field.name for field in meta.fields]
verbose_names = [field.verbose_name for field in meta.fields]
field_dc = dict(zip(verbose_names, field_names))
attr = getattr(User, 'area')
User._meta.get_field('area').related_model
# field.get_internal_type()
# ModelA._meta.get_field(foo).related_model
# attr.get_internal_type()
attr.related_model
attr.__dict__
attr.field.__dict__
attr.field.to_fields
...
class APIResponse(Response):
    def __init__(self, status=200, msg='ok', results=None
...
def my_custom_sql(self):
    # 这样写自动处理了异常，不用手动关闭https://docs.djangoproject.com/en/4.0/topics/db/sql/
    with connection['my_db_alias'].cursor() as cursor:
...
# def my_custom_sql():
#     # 获取该路径下所有excel
#     PATH = "D:\\qqfiles\\642201822\\FileRecv\\张志新0625\\曾超-福建excel\\福清市"
#     tab_name = "fuqing"
#
#     cursor = connection.cursor()
#
#     # 建一张该区县的表
#     sqlstring = f"CREATE table {tab_name} like anxi"
...
def get_user_ip(request):
    ...
    if request.META.get('HTTP_X_FORWARDED_FOR'):
        ...
        print('get from HTTP_X_FORWARDED_FOR')
    else:
        # 正常情况下
        ip = request.META.get("REMOTE_ADDR")
        print('get from REMOTE_ADDR')
    return ip
...
view_num = user_models.UserDynamicDetails.objects.filter(obj_id=OuterRef('pk')).filter(**view_dict)
query_set = query_set.annotate(view_num=Count(Subquery(view_num)))