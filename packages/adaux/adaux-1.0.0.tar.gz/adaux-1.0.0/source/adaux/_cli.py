...
@click.pass_context
@click.option(
    "--target", "-t", type=Path, default="devops", help="Location of the repository."
)
@click.option(
    "--silent",
    "-s",
...
def aux(ctx: click.Context, target: Path, silent: bool) -> None:
...
@click.pass_context
@gitlab_token_option
def release(ctx: click.Context, token: str) -> None:
    r"""
    Create and display release merge request.
    """
    token = _get_token_from_env(token)
    with convert_runtime_to_click_error():
        ctx.obj.release(token)
