...
class StandardMeasurementSet(LuminesceMeasurementFactory):
    ...
    def _writer_test(self, writer, test_data_path, x_rng, name, n_parallel_set, check_period=0.1, keep=90):
        ...
        for n_p in n_parallel_set:
            baseline_cv = Convener(baseline_ex, self.work_dir + '/writers', f'{name}_baseline_NP{n_p}', n_parallel=n_p)
            writer_cv = Convener(writer_ex, self.work_dir + '/writers', f'{name}_NP{n_p}', n_parallel=n_p)
    ...
    def _file_write_test(self, writer, file_type, x_rng, n_cols, name, n_parallel_set, check_period=0.1, keep=90):
        ...
        for n_p in n_parallel_set:
            baseline_cv = Convener(baseline_ex, self.work_dir + '/writers', f'{name}_cols{n_cols}_baseline_NP{n_p}', n_parallel=n_p)
