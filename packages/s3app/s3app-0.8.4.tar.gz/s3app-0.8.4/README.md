...
4. Start S3App server
   ```   
   (s3app) vagrant@bullseye:~$ s3app --host=0.0.0.0 --port=8080
...
# The start parameter
To start of the S3App server simply call `s3app` as shown above. You have has the following parameter:
...
With `s3app -h` you get the possible configuration: 

```
(s3app) vagrant@bullseye:~$ s3app -h
...
usage: s3app [-h] [-s HOST] [-p PORT] [-t THREADS]
