...
if __name__ == '__main__':
    ...
    cursor.execute("select * from store_sales LEFT OUTER JOIN store_returns ON (sr_item_sk = ss_item_sk AND sr_ticket_number = ss_ticket_number);")
    bdata = []
    try:
        for i in range(1000):
            cursor.cancel(None)
    ...
    while True:
        try:
            connection = connect(host="localhost",
                                 ...
                                 thrift_transport=None)
            cursor = Cursor(connection)
            isOpen = cursor.connection.check_connection
            if not isOpen:
                time.sleep(5)
                continue
            else:
    ...
    cursor.execute("SELECT * from catalog_sales limit 100;")
    bdata = []
    for i in range(1000):
        bdata += cursor.fetchmany()
