...
def get_ps_env_var(var_group: str, var_name: str, local_creds: bool = False, prefix: str = None) -> (str, Exception):
    ...
    try:
        ...
        return (client.get_parameter(Name=f'/{prefix}/{var_group.lower()}/{var_name.lower()}', WithDecryption=True)['Parameter']['Value'], None)
...
def create_prefix(prefix: str) -> str:
    ...
    if isinstance(prefix, str):
        return prefix.rstrip('/').lstrip('/')
    return f'{get_sys_env_var().capitalize()}/Tpds'