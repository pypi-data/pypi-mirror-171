...
class loops:
            ...
            while end is False:
                ...
                if channel_messages[0].id != last_scanned.id:
                    ...
                    for x in range(i):
                        ...
                        # catch and raise error if function doesn't take arguments
                        try:
                            function(channel_messages[x])
                        except TypeError:
                            raise autocord.ParameterNotFound(f'Could not find parameter for function {str(function)}')
    ...
    def AUTO_REPLY(self, channel, indicator, response, include_self=False, type=0):
        # indicator is the message that you automatically reply to
        # if a message contains the indicator, it replies
        if type == 0:
            Thread(target=lambda: self.AUTO_REPLY(channel, indicator, response, include_self, type=1)).start()
