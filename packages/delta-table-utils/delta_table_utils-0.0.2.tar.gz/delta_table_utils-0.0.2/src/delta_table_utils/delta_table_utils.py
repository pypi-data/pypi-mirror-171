...
from utils.logger import logger, LogMessage
...
class DeltaTable:
    ...
    def upload_path(self):
        return f"s3://tpg-ds-db-exports/prd/dbname={self.schema_name}/tablename={self.table_name}/"
...
#         dbutils.fs.rm(f"/delta/{self.schema_name}/{self.table_name}", recurse=True)
    ...
    def read_stream(self, spark: SparkSession, **kw):
        ...
        format = kw.get('cloudFiles.format') or 'csv'
    ...
    def stream(self, spark: SparkSession) -> None:
        """
