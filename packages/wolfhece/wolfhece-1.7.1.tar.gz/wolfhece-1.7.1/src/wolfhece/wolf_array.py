...
class Ops_Array(wx.Frame):
    ...
    def OnLoadvec(self, event):
        dlg = wx.FileDialog(None, 'Select file',
                            wildcard='Vec file (*.vec)|*.vec|Vecz file (*.vecz)|*.vecz|All (*.*)|*.*', style=wx.FD_OPEN)
...
class SelectionData():
    ...
    def condition_select(self, cond, condval,usemask=False):
...
class WolfArray(header_wolf):
    ...
    def export_to_gltf(self, bounds=None, fn=''):
        ...
        points = np.asarray(
            [[xx, self.get_value(xx + ox - refx, yy + oy - refy, nullvalue=zmin), -yy] for xx in x for yy in y],
            dtype=np.float32)
    ...
    def plot(self, sx=None, sy=None, xmin=None, ymin=None, xmax=None, ymax=None):
        ...
        if not cursize in self.mygrid.keys():
            ...
            curlist['nbx'] = curnbx
            curlist['nby'] = curnby
            wx.LogMessage(_('OpenGL lists - allocation'))
            numlist = glGenLists(curnbx * curnby)
            curlist['firstlist'] = numlist
            wx.LogDebug(_('OpenGL first list')+str(numlist))
