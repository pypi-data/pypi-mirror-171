...
main = clicks.application("gmpg", gmpg.__doc__)
...
class Analyze:
    ...
    def run(self, stdin, log):
        pyproject = gmpg.get_current_package()
        # TODO fall back to "packages" if "name" fails
        project_name = pyproject["tool"]["poetry"]["name"].replace(".", "_")
