...
class FyersApiBuilder:
  def __init__(self, config, fyers_api):
    self.__set_config(config)
    self.__set_fyers_api(fyers_api)
    self.__generate_folders_if_not_exists()
    self.__initialize()

  def __set_fyers_api(self, config):
    self.__config=FyersApiConfig(config)

  def __set_fyers_api(self, fyers_api):
    self.__accessToken = fyers_api.accessToken
    self.__fyersModel = fyers_api.fyersModel
    self.__Websocket = fyers_api.Websocket
  ...
  def __set_initial_values(self, token):
    ...
    self.client = self.__fyersModel.FyersModel(client_id=self.__config.client_id, token=token, log_path=log_path)

    self.__ws_access_token = f"{self.__config.client_id}:{self.access_token}"
    self.__ws_client = self.__Websocket.ws.FyersSocket(access_token=self.__ws_access_token, run_background=False, log_path=log_path)

  def subscribe(self, symbol = None, data_type = None, custom_message = None):
    self.__ws_client.websocket_data = custom_message  
    self.__ws_client.subscribe(symbol=symbol, data_type=data_type)
    self.__ws_client.keep_running()
  
  def unsubscribe(self, symbol = None):
    self.__ws_client.unsubscribe(symbol=symbol)
  ...
  def __setup(self):
    ...
    session = self.__accessToken.SessionModel(client_id=self.__config.client_id, secret_key=self.__config.secret_key, redirect_uri=self.__config.redirect_uri, response_type="code", grant_type="authorization_code")
