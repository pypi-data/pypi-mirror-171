...
class OverloadDict(dict[typing.Any, OverloadList]):
    ...
    d["a", "b", "c"] = 1, 2, 3
    ```
    Unwanted values may be removed through the `remove_values` method, and can be
    converted into a standard `dict` object through the `to_dict` method.
    ...
    def remove_values(self, remove: typing.Any) -> OverloadDict:
        """Iteratively removes values in each of the contained `OverloadList` objects
        that are equal to `remove` and returns the result as a new `OverloadDict`.
    ...
    def to_dict(self, pref: int = 0) -> dict:
        """Iteratively creates a new `dict` object with the contents if this OverloadDict.
        A list index preference may be chosen through the `pref` parameter, and must be
        either `0` (first item) or `-1` (last item).
