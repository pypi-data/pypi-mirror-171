...
def default_rule_parameters(
    ...
    tool_configuration_keys = set(tool_configuration.keys())

    matching_keys = [k for k in tool_configuration_keys if name in k]

    if name in tool_configuration_keys:
        # an exact match means you provided the exact tool name.
        pass
    elif len(matching_keys) == 1:
        # an non-exact but also non-ambiguous match means you provided a good enough tool name
        name = matching_keys[0]
    else:
        msg = """
        Cannot derive default rule parameters for name: {n}
        Keys in the tool dictionairy: {k}.

        Please verify whether or not this is expected.
        """.format(
            n=name, k=tool_configuration_keys
        )
        print(msg)
        pass
