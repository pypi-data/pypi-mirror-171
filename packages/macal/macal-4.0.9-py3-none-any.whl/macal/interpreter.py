    ...
    def interpretSelect(self, instruction: ast_select.Select, scope: mscope.Scope) -> None:
        ...
        if into is None:
            ...
            if instruction.Distinct:
                into.Value = value_item.ValueItem().setFromMacal(instruction.Token, types.VariableTypes.Record, {})
            else:
                into.Value = value_item.ValueItem().setFromMacal(instruction.Token, types.VariableTypes.Array, [])
