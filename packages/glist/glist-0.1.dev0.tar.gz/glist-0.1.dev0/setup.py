...
# Install repo_utils before we even do setup.
# Determine package_name automatically.
impstall('rc-repo-utils')
from repo_utils import get_repository_path
from os import path
import os

repo_root = get_repository_path(__file__)
package_name = None
for file in os.listdir(repo_root):
    if path.isdir(path.join(repo_root, file)) and path.exists(path.join(repo_root, file, '__meta__.py')):
        package_name = file
if package_name is None:
    raise Exception('Could not find a package folder under repo_root with a __meta__.py file.')
# END repo_utils addition

exec(read(os.path.join(package_name, "__meta__.py")), meta)
...
init_filepaths = enable_installing_dependencies(get_repository_path(__file__), meta["path"])
# END repo_utils addition
