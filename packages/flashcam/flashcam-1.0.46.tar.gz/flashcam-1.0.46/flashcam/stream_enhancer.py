    ...
    def save_avi(self, seconds =  1, name = "" ):
        ...
        if seconds == 0:
            ...
            if not self.aviopened:
                ...
                print("!... getting zmq sender:", self.zmqtarget)
                if not self.zmqtarget is None:
            ...
            if not self.zmqsender is None:
                zmqnow = dt.datetime.now()
                telegramnow = zmqnow

                # telegram
                if os.path.exists(os.path.expanduser("~/.telegram.token")):
                    if (telegramnow-self.telegramlast).total_seconds()>300:
                        y = threading.Thread(target=telegram.bot_send, args=("ALERT", f"CAMERA {self.zmqrpi_name} {zmqnow.strftime('%a %H:%M:%S')}", self.frame))
                        y.start()
                ...
                if (zmqnow-self.zmqlast).total_seconds()>1.5:
                    ...
                    self.zmqlast=zmqnow
                    print(f"i... zmq is being sent to {self.zmqtarget}")
