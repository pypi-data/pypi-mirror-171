...
def bus(model, name, energy_vector="Electricity", **kwargs):
    r"""
    ...
    bus.energy_vector = energy_vector
...
def transformer_constant_efficiency_optimize(model, dict_asset, **kwargs):
    ...
    if missing_dispatch_prices_or_efficiencies is None:
        ...
        ###Check if it's an extraction turbine:
        if BETA in dict_asset:
            ext_turb_chp(model, dict_asset, **kwargs)
        else:
...
def extraction_turbine_chp(model, dict_asset, **kwargs):
    r"""
    ...
    # electricity efficiency: e_1 - beta *e_2
    # thermal efficiency: e_2
    # full condensation: e_1

    efficiencies_full_condesation = {
        kwargs[OEMOF_BUSSES][dict_asset[OUTFLOW_DIRECTION][0]]: dict_asset[EFFICIENCY][
            VALUE
        ][0]
    }

    # if BETA in dict_asset: #it's actually now checked before in the transformer but here again
    #     Beta = dict_asset[BETA][VALUE]
    # else:
    #     raise ValueError(
    #         "no beta for extraction turbine chp specified. It should be a number between 0 and 1."
    #     )

    Beta = dict_asset[BETA][VALUE]

    efficiencies = {}
    efficiencies[kwargs[OEMOF_BUSSES][dict_asset[OUTFLOW_DIRECTION][0]]] = dict_asset[
        EFFICIENCY
    ][VALUE][0]
    -Beta * dict_asset[EFFICIENCY][VALUE][1]
    efficiencies[kwargs[OEMOF_BUSSES][dict_asset[OUTFLOW_DIRECTION][1]]] = dict_asset[
        EFFICIENCY
    ][VALUE][1]
    ...
    ext_turb_chp = solph.components.ExtractionTurbineCHP(
        ...
        outputs=outputs,
        conversion_factors=efficiencies,
        conversion_factor_full_condensation=efficiencies_full_condesation,
    )
