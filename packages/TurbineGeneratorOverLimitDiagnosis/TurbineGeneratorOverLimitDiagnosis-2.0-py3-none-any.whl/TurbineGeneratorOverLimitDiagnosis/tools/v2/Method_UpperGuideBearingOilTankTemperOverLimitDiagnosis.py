...
class UpperGuideBearingOilTankTemperOverLimitDiagnosis(MultipleThresholds_GreaterEqualTo_LesserThan):
    def __init__(self, *args, **kwargs):
        """
        上导轴承油槽温度超限判断

        :math:`T < 45, 上导轴承油槽温度不超限`

        :math:`45 ≤ T < 50, 上导轴承油槽温度超限一级报警`

        :math:`T ≥ 50, 上导轴承油槽温度超限二级报警`


        [1] 参数
        ----------
        thresholds:
            list[float],报警限,默认[45, 50]
        truthTable:
            dict,报警真值表,形如:
                {
                    "100": {"dfem_evidence": "[上导轴承油槽温度]正常", "dfem_code": ""},
                    "010": {"dfem_evidence": "[上导轴承油槽温度]实测值(measured)超限一级报警,阈值threshold", "dfem_code": "SP000441"},
                    "001": {"dfem_evidence": "[上导轴承油槽温度]实测值(measured)超限二级报警,阈值threshold", "dfem_code": "SP000442"},
                }
