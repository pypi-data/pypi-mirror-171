...
class PyMedQuery:
    ...
    def upload(
        ...
        # you need to start accounting for datetime objects as they will fail on other if clauses
        # This ugly duckling is written due to the variabilty of input values and that we are currently
        # reveiving dicts for handling the request. The forward backward mutation is not particularly a good one.
        # NOTE! Solve this matter in way where we don't have to mutate values.
        record_values_lst = [
            rec if isinstance(rec, datetime) else list(rec)
            if isinstance(rec, tuple) else int(rec)
            if rec.isdigit() else float(rec)
            if re.match(r'^-?\d+(?:\.\d+)$', rec) else dict(rec)
            if '{' in rec else rec
