...
class TaskResult:
    ...
    solutions: List[Solution]  # sorted by confidence
...
def get_results(
    ...
    for assignment, solutions in markup_assignments:
        for input_objects, output_objects in solutions:
            ...
            if task_id not in task_id_to_result:
                task_id_to_result[task_id] = TaskResult(input_objects, solutions=[])
            ...
            task_id_to_result[task_id].solutions.append(solution)

    for result in task_id_to_result.values():
        result.solutions = sorted(result.solutions, reverse=True)
