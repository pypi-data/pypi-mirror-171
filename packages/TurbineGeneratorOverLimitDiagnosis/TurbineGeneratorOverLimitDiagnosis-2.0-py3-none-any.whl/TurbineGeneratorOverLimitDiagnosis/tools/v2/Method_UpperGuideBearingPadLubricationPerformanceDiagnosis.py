...
class UpperGuideBearingPadLubricationPerformanceDiagnosis:
    def __init__(self, *args, **kwargs):
        """
        上导轴承瓦面润滑性能异常评估

        :math:`T < 1.2, 上导轴承瓦面润滑性能无异常`

        :math:`1.2 ≤ T, 上导轴承瓦面润滑性能异常`


        [1] 参数
        ----------
        thresholds:
            list[float],报警限,℃/min,默认[1.2]
        truthTable:
            dict,报警真值表,形如:
                {
                    "10": {"dfem_evidence": "[上导轴承瓦面润滑性能]正常", "dfem_code": ""},
                    "01": {"dfem_evidence": "[上导轴承瓦面润滑性能]轴瓦温度上升率计算值(measured)超限报警,阈值threshold", "dfem_code": "SP000500"},
                }
    ...
    def diagnosis(self, padTemper: float, timestamp: float):
        ...
        判据是上导轴瓦上升率℃/min,即,一分钟内第一个最小值与最后一个最大值的差值
        默认缓存时间长度为120s
