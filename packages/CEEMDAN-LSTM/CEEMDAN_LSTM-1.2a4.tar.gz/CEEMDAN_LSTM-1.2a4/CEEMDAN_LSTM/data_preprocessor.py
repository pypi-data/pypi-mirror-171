...
def decom(series=None, decom_mode='ceemdan', vmd_params=None, **kwargs): 
    """
...
def decom_ovmd(series=None, vmd_params=None, trials=100): # VMD Decomposition
    ...
    if vmd_params is None:
        try: import optuna
        except: raise ImportError('Cannot import hyperopt, use pip install optuna!')
        def objective(trial):
...
def redecom(data=None, show_data=False, decom_mode='ceemdan', inte_list='auto', redecom_list={'co-imf0':'ovmd'}, vmd_params=None, **kwargs):
    """
    ...
    **kwargs        - any parameters of PyEMD.EMD(), PyEMD.EEMD(), PyEMD.CEEMDAN(), vmdpy.VMD()
    ...
    if len(data.columns) == 1: 
        df_decom = decom(data[data.columns[0]], decom_mode=decom_mode)
    ...
    if redecom_list is not None:
        ...
        for i in redecom_list.columns:
            ...
            else: df_redecom = decom(df_inte[i], decom_mode=redecom_list[i][0], vmd_params=vmd_params[i], **kwargs)
