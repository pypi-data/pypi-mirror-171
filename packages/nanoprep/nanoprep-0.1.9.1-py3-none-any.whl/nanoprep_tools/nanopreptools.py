...
def nano_groupreads(mapping, OUTPUT="Grouped_reads", groupfile=None, keepSubdir=False, changeName=True, changePrefix="", gzip_extension=False, exist_OK = False):
            ...
            for mread in reads_need_moving:
                ...
                if keepSubdir:
                    ...
                    if changeName: # change subfolder name
                        ...
                        # sam=sample path
                        new_name =k+"_"+changePrefix+sampleid+"_"+bar
                        sub_sam = os.path.join(sam, new_name)
                        if gzip_extension:
                            chgn_read = os.path.join(sub_sam, new_name+"."+os.path.basename(mread).split(".")[1]+".gz")
                        else:
                    ...
                    if changeName: # change subfolder name
                        new_name =k+"_"+changePrefix+sampleid+"_"+bar
                        if gzip_extension:
                            chgn_read = os.path.join(sam, new_name+"."+os.path.basename(mread).split(".")[1]+".gz")
                        else:
