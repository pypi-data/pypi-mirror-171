...
class Interpreter:
    ...
    def InterpretSelect(self, instruction: ast_select.Select, scope: mscope.Scope) -> None:
        ...
        if data is not None and isinstance(data.Value, value_item.ValueItem):
            raise exceptions.RuntimeError(f'Debug Select line 1: data.Value is ValueItem but it should not be!', data.Value.Token.Location, self.filename)
        ...
        if into is None:
            ...
            into = intoVar.Value
        if isinstance(into.Value, value_item.ValueItem):
            raise exceptions.RuntimeError(f'Debug Select line 2: into.Value is ValueItem but it should not be!', into.Value.Token.Location, self.filename)
        if instruction.Where is not None and data is not None:
            data = self.InterpretSelectWhere(instruction.Where, data, scope)
        if data is not None and isinstance(data.Value, value_item.ValueItem):
            raise exceptions.RuntimeError(f'Debug Select line 3 (where): data.Value is ValueItem but it should not be!', data.Value.Token.Location, self.filename)
        if not(len(instruction.Fields) == 1 and instruction.Fields[0].Name == '*'):
            data = self.ApplyFieldFilters(data, instruction.Fields)
        if data is not None and isinstance(data.Value, value_item.ValueItem):
            raise exceptions.RuntimeError(f'Debug Select line 4 (apply field filters): data.Value is ValueItem but it should not be!', data.Value.Token.Location, self.filename)
        if instruction.Distinct is True and data is not None and data.Type == types.VariableTypes.Array and len(data.Value) > 0:
            data = data.Value[0]

        if data is not None and isinstance(data.Value, value_item.ValueItem):
            raise exceptions.RuntimeError(f'Debug Select line 5 (distinct): data.Value is ValueItem but it should not be!', data.Value.Token.Location, self.filename)

        if data is not None and data.Type == types.VariableTypes.Array and len(data.Value) == 1:
            # it should be an array of value item record, so this is the way to do it.
            data = data.Value[0]

        if data is not None and isinstance(data.Value, value_item.ValueItem):
            raise exceptions.RuntimeError(f'Debug Select line 6 (array of 1): data.Value is ValueItem but it should not be!', data.Value.Token.Location, self.filename)

        if instruction.Merge is True and data is not None:
            data = self.InterpretSelectMerge(into, data)
        
        if data is not None and isinstance(data.Value, value_item.ValueItem):
            raise exceptions.RuntimeError(f'Debug Select line 7 (merge): data.Value is ValueItem but it should not be!', data.Value.Token.Location, self.filename)
        ...
        if data is not None and isinstance(data.Value, value_item.ValueItem):
            raise exceptions.RuntimeError(f'Debug Select line 8 (array of one 2): data.Value is ValueItem but it should not be!', data.Value.Token.Location, self.filename)
        if data is not None:
