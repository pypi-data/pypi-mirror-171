...
class HangmanTestCase(unittest.TestCase):
    ...
    def test_length(self):
        self.assertIn('word_list', self.vars, 'The word_list variable is not defined, if you have defined it, make sure the name is correct and it is in milestone_1.py')
    ...
    def test_random(self):
        ...
        finder = ModuleFinder()
        finder.run_script('milestone_1.py')
        self.assertIn('random', finder.modules, 'The random module is not imported in milestone_1.py. Make sure you import it to pick a random word from the list')
        # Check if the random.choice function is called
        self.assertIn('choice', self.clean_vars.values(), 'The random.choice function is not called in milestone_1.py. Use it to pick a random word from the list')
    ...
    def test_presence_readme(self):
        ...
        with open('README.md', 'r') as f:
            readme = f.read()
        self.assertGreater(len(readme), 100, 'The README.md file should be at least 1000 characters long')
