...
class Account(BaseDataClass['AccountResponse']):
    ...
    id: int  
    username: str  #: An account's unique username
    #: An account's display name
    display_name: str  
    #: An account's profile image name
    profile_image: str 
    #: An account's junior status 
    is_junior: bool  
    #: An account's logged platforms
    platforms: List[str]  
    #: An account's set pronouns
    personal_pronouns: List[str]  
    #: An account's set identities
    identity_flags: List[str]  
    #: An account's exact creation date as an Unix timestamp
    created_at: int  
    #: An account's RecNet banner image name
    banner_image: Optional[str] = None  
    #: An account's bio
    bio: Optional[str] = None  
    #: An account's level and XP
    level: Optional[Progression] = None  
    #: An account's subscriber count
    subscriber_count: Optional[int] = None 
    #: An account's influencer status 
    is_influencer: Optional[bool] = None 
    #: An account's hosted events
    events: Optional[List['Event']] = None  
    #: An account's created rooms
    created_rooms: Optional[List['Room']] = None  
    #: An account's owned rooms
    owned_rooms: Optional[List['Room']] = None  
    #: An account's shared images
    images: Optional[List['Image']] = None  
    #: An account's RecNet feed
