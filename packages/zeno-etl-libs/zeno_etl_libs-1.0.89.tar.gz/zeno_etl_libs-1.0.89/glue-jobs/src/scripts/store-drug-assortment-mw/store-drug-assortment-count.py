...
try:
        ...
        df_q = """
                 select
                    ...
                    count(distinct case when i.quantity>0 then sda."drug-id" end) "available-count"
                   
                from
                    "prod2-generico"."store-drug-assortment" sda
                    left join "prod2-generico".drugs d
                    on sda."drug-id" = d.id 
                    left join (select "drug-id",sum(quantity) quantity from "prod2-generico"."inventory-1" i 
