...
def hist(
        ...
        with_histogram_dither=False, hist_dith_factor=4, orientation="vertical", hide_x_labels=False,
        color=matplotlib.cm.get_cmap('Blues')(0.95), ymarkers=None, ylim=None):
    ...
    print('Y granularity is {:.2e}, scale range is {:.2f} bits'.format(ygran, np.log2(yspan/ygran)))

    unique_vals = np.unique(yarr)
    ...
    if len(unique_vals) < target_num_bins:
        ...
        bins = np.arange(yrng[0], yrng[1], ygran*ygrans_per_bin*hist_bin_sz_fac)

    num_bins = len(bins)

    if abs(len(bins)/target_num_bins - 1) > 0.1:
        print("Number of histogram bins requested: {:d}, actual number is {:d} due to data granularity".format(target_num_bins, num_bins))
    ...
    plt.hist(yarr*dith, bins=bins, orientation=orientation, color=color, zorder=10, log=False)
    ax = plt.gca()
...
def add_marker(
        pos=None, label=None, axis='x', c='k', ls='--', zorder=None, xlim=None, verbose=False,
        ylim=None, label_pos='left', label_vpos='bottom', scale_type=None, **kwargs):
    ...
    if label is not None:
        ...
        if axis == 'x':
            ...
            if yscale_type == 'linear':
                ...
                if label_vpos == 'bottom':
                    ypos = 10**(np.log10(pos) - np.log10(yspan)*0.01)
                    label_va = 'top'
                elif label_vpos == 'top':
                    ypos = 10**(np.log10(pos) + np.log10(yspan)*0.01)
                    label_va = 'bottom'
            ...
            if scale_type == 'linear':
                ...
                if label_pos == 'left':
                    xpos = 10**(np.log10(xlim[0])+np.log10(xspan)*0.05)
                    label_ha = 'left'
                else:
                    xpos = 10**(np.log10(xlim[0])-np.log10(xspan)*0.05)
                    label_ha = 'right'
...
def occurrence_plot(
        ...
        show_avg_trace=False, yl=None, yl_frac=None, ymarker=None,
        show_zero_line=True, show_unity_line=False, ymarker_label=None, xlim=None,
    ...
    if force_plot_sample_trace or (isnone(sliced_data) and len(X) < MIN_PTS_FOR_OCCURRENCE_PLOT):
    ...
    if(isnone(sliced_data)):
        ...
        if isnone(extra_bin) and len(Xb) > MAX_SLICED_LENGTH:
            print("Sliced dataset length is {:d} which larger than the maximum {:d}".format(len(Xb), MAX_SLICED_LENGTH))
            extra_bin = int(len(Xb)/300)
            print("Optimal sliced length is {:d}, setting extra_bin to {:d}".format(OPTIMAL_SLICED_LENGTH, extra_bin))

        if not isnone(extra_bin):
            Xb2 = []
            Yb2 = []
            Yb2_lvls = np.ndarray([int(np.ceil(Yb_lvls.shape[0]/extra_bin)), Yb_lvls.shape[1]])
            ind2 = 0
        ...
        #bin_inds = np.round(np.linspace(0, len(Xb), target_num_points)).astype('int')
        #bin_inds[-1] = len(Xb) - 1
    ...
    if(isnone(yl)):
        ...
        if(not isnone(yl_frac)):
            ...
            # with an extra 10%
            if not isnone(Y):
                yl = extend_range([np.nanmin(Y), np.nanmax(Y)], 0.1)
            else:
                yl = extend_range([np.nanmin(Yb_lvls), np.nanmax(Yb_lvls)], 0.1)

    #print("Y axis range: {:.2f}, {:.2f}".format(yl[0]/mean_y, yl[1]/mean_y))
    ...
    # So the average trace is medium red with sliced data, and dark blue when on
    # its own or when merge_slice_regions is enabled.
        ...
        for ind, ym in enumerate(ymarker):
            ...
            if ind == 0:
                ...
                if ymarker_label is not None:
                    ...
                    print("Y marker given, but no label specified, assuming it is RSD range")
    ...
    if(with_hist):
        ...
        if len(unique_vals) < target_hist_bins:
            ...
            y_grans_per_bin = np.max([np.round(num_hist_bins/target_hist_bins), 1])
            bins = np.arange(yl[0], yl[1], y_gran*y_grans_per_bin*hist_bin_size_fac)

        num_bins = len(bins)

        if(abs(len(bins)/target_hist_bins - 1) > 0.1):
            print("Number of histogram bins requested: {:d}, actual number is {:d} due to Y data granularity".format(target_hist_bins, num_bins))
        ...
        plt.hist(Y*dith, bins=bins, orientation="horizontal", color=hist_color, zorder=10)[0]

        if not isnone(hist_ymarkers):
            # Add histogram y markers
            for hist_ymarker in hist_ymarkers:
                add_y_marker(hist_ymarker, c=[0.75, 0.75, 0.75], ls='-', zorder=1)
