...
def _get_data_sources(args: FeatureViewArgs) -> List[specs.DataSourceSpec]:
    ...
    for ds_id in ds_ids:
        ds = _ALL_FCOS[ds_id]
        data_source_specs.append(_spec_from_declarative_data_source(ds))

    return data_source_specs


@typechecked
def _spec_from_declarative_data_source(ds: BaseDataSource) -> specs.DataSourceSpec:
    ...
    # Need to set a dummy URI because the data source spec expects the batch source to be one
    # of a file, hive, etc. source.
    data_proto.batch_data_source.file.uri = ""
    return specs.DataSourceSpec.from_data_proto(data_proto)
