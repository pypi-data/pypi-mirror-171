...
check_date() parameters and methods
    ...
    1)  compare('date1','operator','date2') = Compare two date(==, <, <=, >, >=)
        1.1)  compare('date1', '==', 'date2') = Check wether the date1 and date2 are equal or not
...
check_date() constraints
    ...
    3)  isleapyear(year) have only one parameter that is an year.
 ...
 **Usecase 3** :- Check wether the date1 is less than or equalto date2
