...
class BaseModel(db.Model):
    ...
    def _construct_filters_from_query_params(query_params):
        """
        Each filter parameter has a filter modifier (starts_with,
        ends_with, etc.) and a field name. If a modifier is not
        present, `equals` is used as the default modifier.

        The filter parameter takes the following format:
            __<filter_modifier>__<field_name>

        For example, if you want to get users who are younger than 20,
        you can use the filter parameter `__lesser_than__age` where
        `lesser_than` is the modifier and `age` is the field name.
        ...
        for key in query_params:
            ...
            # If operator modifier is not applied, use `equals` by default
            if len(results) == 1:
    ...
    # Verified
    @staticmethod
    def add_filters_to_query(q, fields, filters):
        logger.info(filters)
        for f in filters:
            if f['name'] in fields:
                ...
                elif fields[f['name']]['type'] == 'BOOL' or fields[f['name']]['type'] == 'ENUM':
                    for filter_ in f['filters']:
    ...
    # Verified
    # TODO: This doesn't do any joins
    ...
    @classmethod
    def get_all_objects_details(cls, joins=[], status='active', **params):
        """
        Pass status=None if you don't want the status filter to be applied.
        """
