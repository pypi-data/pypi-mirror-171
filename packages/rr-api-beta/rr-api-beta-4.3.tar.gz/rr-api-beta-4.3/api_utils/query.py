...
class Players:
    ...
    def player_basic_info(object_id: int, seasons=None):
        ...
        return "select rpr.player_id, rpr.player_name, rpr.position, rpm.age, rpm.place_of_birth, rpm.contract_expires, " \
               ...
               "ELSE 0 END as decimal (10)) as mv, rpr.final_rank " \
               ...
               "and rpm.player_id = rpr.player_id limit 1;"
    ...
    def rank_similar_players(object_id: int, position: str, player_rank, seasons=None):
        ...
        if player_rank is not None and type(player_rank) is not float:
            ...
            # rp.rankset_id, rp.rankset_name, rpr.team_name, rpr.tour_name, rpr.position, rpr.player_rank, " \
            #                     "
            # rpr.final_rank, rpr.player_rank_with_stats, rpr.tour_rank, rpr.team_rank, rpr.minutes_played, rpr.minutes_played_new, " \
            #                     "
            # rpr.season, rpm.age, rpm.place_of_birth, rpm.mv, rpm.mv_type
        return "select rpr.player_id as rankset_id, rpr.player_name, rpm.age, cast(rpr.final_rank as decimal (10,2)) AS total_rank, rpm.place_of_birth, rpm.contract_expires, " \
               "cast(CASE WHEN rpm.mv_type = 'Th.' THEN rpm.mv * 1000 " \
               ...
               f"and rpr.final_rank between {round(player_rank - 1, 2)} and {round(player_rank + 1.5, 2)}) " \
               f"and rpm.player_id = rpr.player_id and rpr.season in ('{seasons[0]}', '{seasons[1]}') order by total_rank desc;"
...
class Ranking:
    ...
    def players_ranking(data_type: int, object_id: int = None):
        main_query = "select distinct rp.rankset_id, rp.rankset_name as player_name, rpr.team_name, rpr.position, rpr.tour_name, cast(rpr.player_rank as decimal (10,2)) AS player_rank," \
                     "cast(rpr.player_rank_with_stats as decimal (10,2)) AS stats_rank, cast(rpr.final_rank as decimal (10,2)) AS total_rank, rpr.tour_rank, rpr.team_rank," \
                     ...
                     "from rankset.players as rp, rankset.teams as rt, rankset.countries as rc, rankset.player_rank as rpr, rankset.player_metadata as rpm " \
                     "where rt.rankset_id = rp.main_id and rc.rankset_id = rt.main_id and rpr.player_id = rp.rankset_id and rpm.player_id = rpr.player_id and season like '%/%'"
    ...
    def player_rank(player_id: int):
        return f"select distinct rp.rankset_id, rp.rankset_name, cast(rpr.player_rank as decimal (10, 2)) as avg_rank " \
