...
class ManifestImprover(
    ...
    def improve_descendants_from_name(self, keys: list[str] | None = None):
        sources = [t for t in self.titles if t != "title"]
        targets = [self.replace_suffix(t, suffix="descendants") for t in sources]

        source_values = sources  # input is actual name
        target_values = sources  # input is actual name
        if keys is None:
            keys = [
                k
                for k in self.enrichments
                if any(
                    [
                        ...
                        for s in {
                            ...
                            "descendants_from_name",
                            "descendant_from_names",
                            "descendants_from_names",
                        }
                    ]
                )
            ]
