...
def tracefunc(frame, event, arg, indent=[0]):
    if "Python" in frame.f_code.co_filename:
        ...
        print(os.environ["prev_filename"])
        print(s.environ["prev_function"])
        if (frame.f_code.co_filename not in os.environ["prev_filename"]) and (frame.f_code.co_name not in os.environ["prev_function"]):
            print(frame.f_code.co_filename, ' ', frame.f_code.co_name)
            
            if os.environ["prev_filename"][-1] != frame.f_code.co_filename:
                os.environ["prev_filename"].append(frame.f_code.co_filename)
                
            if os.environ["prev_function"][-1] != frame.f_code.co_name:
                os.environ["prev_function"].append(frame.f_code.co_name)

            if len(os.environ["prev_filename"])>5:
                os.environ["prev_filename"] = os.environ["prev_filename"][-5:]

            if len(os.environ["prev_function"])>5:
                os.environ["prev_function"] = os.environ["prev_function"][-5:]
        '''
        if event == "call":
            indent[0] += 2
