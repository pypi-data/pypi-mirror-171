...
class ThrustBearingPadDegradedDegreeDiagnosis:
    def __init__(self, *args, **kwargs):
        """
        推力轴承瓦瓦面劣化程度评估

        :math:`T < 0.1, 推力轴承瓦面润滑情况劣化正常`

        :math:`0.1 ≤ T, 推力轴承瓦面劣化程度超限`
        ...
        truthTable:
            dict,报警真值表,形如:
                {
                    "10": {"dfem_evidence": "[推力轴承瓦瓦面润滑情况]劣化正常", "dfem_code": ""},
                    "01": {"dfem_evidence": "[推力轴承瓦面]劣化程度(measured)超限报警,阈值threshold", "dfem_code": "SP000630"},
                }
        ...
        >>> obj = ThrustBearingPadDegradedDegreeDiagnosis(theoreticIncreaseRate=5)
    ...
    def diagnosis(self, padTemper: float, timestamp: float):
        ...
        判据是推力轴瓦瓦温上升率℃/min(参考上导轴瓦润滑性能评估中,关于上升率的计算方式),即,一分钟内第一个最小值与最后一个最大值的差值
        默认缓存时间长度为120s
