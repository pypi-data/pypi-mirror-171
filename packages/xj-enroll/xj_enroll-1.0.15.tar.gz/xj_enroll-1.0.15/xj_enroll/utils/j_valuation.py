...
class JBaseExpression:
    ...
    def get_child_info():
        try:
            return {getattr(i, "name"): i for i in JBaseExpression.__subclasses__() if not getattr(i, "name", None)}, None
...
class JExpression(JBaseExpression):
    def process(self, expression_string):
        ...
        brackets_twain_index = self.parsed_brackets(expression_string)
        child_class = self.get_child_info()
        print(child_class)
        # 运行公式
        try:
