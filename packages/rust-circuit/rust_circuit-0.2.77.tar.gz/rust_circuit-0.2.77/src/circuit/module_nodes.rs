...
use super::circuit_utils::has_leaf_symbols;
use super::{
    deep_map_op_context, deep_map_op_context_preorder_stoppable, deep_map_op_pre_new_children,
    deep_map_unwrap, deep_map_unwrap_preorder, prelude::*, visit_circuit, Add, CachedCircuitInfo,
        ...
        _ => {
            if inputs[..] == circuit.children().collect::<Vec<_>>()[..] {
                ...
                Err(CircuitConstructionError::ExpandNodeUnknownVariant {
                    variant: circuit.variant_string().to_owned(),
                })
            }
        }
    }
...
impl ModuleNodeSpec {
    ...
    pub fn new_auto(
        ...
        let circuit = deep_map_op_context(
            &circuit,
            &|sub: &Circuit, input_specs_dict: &mut HashMap<usize, ModuleNodeArgSpec>| {
    ...
    pub fn resize(&self, shapes: Vec<Shape>) -> Result<Self, CircuitConstructionError> {
        let input_specs: Vec<ModuleNodeArgSpec> = zip(&self.input_specs, shapes)
            .map(|(spec, shape)| ModuleNodeArgSpec {
                symbol: Symbol::new(shape, spec.symbol.uuid.clone(), spec.symbol.name_cloned()),
...
pub fn inline_all_modules(circuit: CircuitRc) -> CircuitRc {
    deep_map_unwrap(&circuit, |c| match c {
        Circuit::ModuleNode(mn) => inline_all_modules(mn.expand().clone()),
        _ => c.clone().rc(),
    })
}
