...
def _func(inletWaterTemper, outletWaterTemper, coldOilTemper, hotOilTemper, Ql=24.3, Qw=4.85):
    Tco1 = hotOilTemper - 2.1 * Qw * (outletWaterTemper - inletWaterTemper) / Ql
    return (coldOilTemper - Tco1) / Tco1


class ThrustBearingOilCoolerDiagnosis:
    def __init__(self, **kwargs):
        ...
        ----------
        func:
            str,冷油温度实测值与计算值偏差,默认为Tco1=Thi-2.1*Qw*（Two-Twi）/Ql,可以通过完整的函数进行定义.推力油冷却器进水温度Twi、出水温度Two、冷油温度Tco、热油温度Thi、冷却水流量Qw
        truthTable:
            dict,判断真值表,其中,"i"、"(i)"不可变,默认:
                {
                    "10": {"dfem_code": "", "dfem_evidence": "[推力轴承油冷却器]运行正常"},
                    "01": {"dfem_code": "SP000580", "dfem_evidence": "[推力轴承油冷却器]运行异常,冷油温度计算值偏差(measured),阈值threshold"},
                }
        threshold:
            float,偏差门限值,默认0.1
