...
def parse_charges_func(args):
    ...
    cells.write_molcas_basis(
        scell.atom_labels, without_central_indices, supercell_charges, "ENV")

    supercell_labels = [
        "{}.ENV.{}".format(lab, it + 1) if it in without_central_indices else
        lab for it, lab in enumerate(scell.atom_labels)]
