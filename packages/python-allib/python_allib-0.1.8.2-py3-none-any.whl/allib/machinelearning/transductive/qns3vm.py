...
class QN_S3VM_Dense:
    ...
    def __initializeMatrices(self):
        if self.__matrices_initialized == False:
            ...
            for l in self.__L_l:
                x.append(l)
            self.__YL = np.mat(x, dtype=np.float64)
            ...
            self.__Xreg = (np.mat(self.__X)[self.__regressors_indices,:].tolist())
            ...
            self.__X_u_subset = (np.mat(self.__X_u)[subset_unlabled_indices,:].tolist())
    ...
    def __getFitness(self,c):
        ...
        preds_labeled = self.__surrogate_gamma*(1.0 - np.multiply(self.__YL, self.__KLR * c_new + b))
    ...
    def __getFitness_Prime(self,c):
        ...
        term1 = np.multiply(preds_labeled_exp, 1.0/(1.0 + preds_labeled_exp))
        term1 = np.multiply(preds_labeled_good_indicator, term1)
        # Replace critical values with "1.0"
        term1 = term1 + preds_labeled_conflict_indicator
        term1 = np.multiply(self.__YL, term1)
        preds_unlabeled_squared_exp_f = np.multiply(preds_unlabeled,preds_unlabeled)
        preds_unlabeled_squared_exp_f = np.exp(-self.__s * preds_unlabeled_squared_exp_f)
        preds_unlabeled_squared_exp_f = np.multiply(preds_unlabeled_squared_exp_f, preds_unlabeled)
...
class LinearKernel():
    ...
    def computeKernelMatrix(self, data1, data2, symmetric=False):
        ...
        self._data1 = np.mat(data1)
        self._data2 = np.mat(data2)
...
class DictLinearKernel():
    ...
    def computeKernelMatrix(self, data1, data2, symmetric=False):
        ...
        self.__km = None
        try:
            km = np.mat(np.zeros((self._dim1, self._dim2), dtype=np.float64))
            if self._symmetric:
    ...
    def computeKernelMatrix(self, data1, data2, symmetric=False):
        ...
        try:
            if self._symmetric:
                ...
                trnorms = np.mat(np.diag(linearkm)).T
                trace_matrix = trnorms * np.mat(np.ones((1, self._dim1), dtype = np.float64))
                ...
                linkm = np.mat(self._data1 * self._data2.T)
                trnorms1 = []
                for i in range(m):
                    trnorms1.append((self._data1[i] * self._data1[i].T)[0,0])
                trnorms1 = np.mat(trnorms1).T
                trnorms2 = []
                for i in range(n):
                    trnorms2.append((self._data2[i] * self._data2[i].T)[0,0])
                trnorms2 = np.mat(trnorms2).T
                self.__km = trnorms1 * np.mat(np.ones((n, 1), dtype = np.float64)).T
                self.__km = self.__km + np.mat(np.ones((m, 1), dtype = np.float64)) * trnorms2.T
