...
def parse_param(input_syntax):
    ...
    result = result + (f"..*['{eco}']" if 'eco' in eco else f"..*[@.name is '{eco}']") + (f"..*['{driver}']" if 'driver' in driver else f"..*[@.class is '{driver}' or @.subclass is '{driver}']") + f".{param_expression}.filename"
...
def get_input_file_path(data, expression, env_dir):
    ...
    # Reads eco from ..*['eco_123'] or ..*[@.name is 'EUR'] 
    # and driver from ..*['driver_123'] or ..*[@.class is 'IR' or @.subclass is 'RIR']
    ...
    if 'eco' in eco_name:
        eco_name = query(data, f"$..*['{eco_name}']")[0]['name']
        logger.debug(f"found eco_name as {eco_name}")
    
    if 'driver' in driver_name:
