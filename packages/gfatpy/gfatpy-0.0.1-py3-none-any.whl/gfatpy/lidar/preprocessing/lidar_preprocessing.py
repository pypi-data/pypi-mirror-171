...
def preprocess(
    ...
    Args:
        ...
        crop_ranges (tuple[float, float] | None, optional): _description_. Defaults to (0, 20000).
        background_ranges (tuple[float, float] | None, optional): _description_. Defaults to None.
        apply_dc (bool, optional): _description_. Defaults to True.
        apply_dt (bool, optional): _description_. Defaults to True.
        apply_bg (bool, optional): _description_. Defaults to True.
        apply_bz (bool, optional): _description_. Defaults to True.
        save_dc (bool, optional): _description_. Defaults to False.
        save_bg (bool, optional): _description_. Defaults to False.
        gluing_products (bool, optional): _description_. Defaults to False.
    ...
    if not _p.exists():
        warnings.warn(f"Not found {_p}, looking for it in NAS")
        _p = filename2path(_p.name, root_dir=DATA_DN, check_exists=True)

    dataset = xr.load_dataset(_p)
    dataset = drop_unwanted_channels(dataset, channels=channels)

    if apply_dc:
        dataset = apply_dark_current_correction(dataset, rs_path=_p, save_dc=save_dc)
    ...
    # dataset = apply_polarization_merge(dataset)
