...
var FoldMode = exports.FoldMode = function (markers) {
    this.foldingStartMarker = new RegExp("([\\[{])(?:\\s*)$|(" + markers + ")(?:\\s*)(?:#.*)?$|([\'\"]{3}[\\s]*@blockdoc)|([\'\"]{3}[\\s]*\u2800)");
};
...
(function () {
    this.getFoldWidgetRange = function (session, foldStyle, row) {
        ...
        if (match) {
            ...
            if (match[3]) {
                return this.quoteBlock(session, row, match.index + match[3].length);
            }
            if (match[4]) {
                return this.quoteBlock(session, row, match.index + match[4].length);
            }
