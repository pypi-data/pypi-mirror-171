...
class CommunicationSimulator(mosaik_api.Simulator):
    ...
    def receive_messages_from_omnetpp(self, time, messages_sent):
        """
            ...
            if len(msgs) == 0:
                if not initial_waiting_msg_sent:
                    if not messages_sent:
                        self.send_waiting_msg(sim_time=time + 1)
                ...
                if send_waiting_msg:
                    ...
                    if not messages_sent:
                        ...
                        if not next_step:
                            ...
                            self.send_waiting_msg(sim_time=next_step + 1)
    ...
    def send_waiting_msg(self, sim_time):
        ...
        log(f'Send WaitingMessage_{self._waiting_msgs_counter} at time {sim_time}')
    ...
    @staticmethod
    def check_msgs(messages, time, reached_until):
        """
        ...
        for message in messages:
            if (type(message) is InfoMessage) or (type(message) is InfrastructureMessage):
                ...
                elif message_time != message.sim_time and not reached_until:
    ...
    def step(self, time, inputs, max_advance):
        ...
        if time == max_advance:
            return None
        # TODO: discuss: only call this method if waiting for msgs
        answers, next_step = self.receive_messages_from_omnetpp(time, messages_sent)
        ...
        if received_messages:
            ...
            received_messages = self.check_msgs(received_messages, time, time < self.mosaik.world.until)
