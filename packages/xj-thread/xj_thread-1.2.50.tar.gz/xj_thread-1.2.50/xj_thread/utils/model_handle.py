...
def format_params_handle(param_dict, filter_filed_list=None, remove_filed_list=None, alias_dict=None):
    ...
    if filter_filed_list and isinstance(filter_filed_list, list):
        param_dict = {k: v for k, v in param_dict.copy().items() if k in filter_filed_list}

    # 类型判断， 剔除字段
    if remove_filed_list and isinstance(remove_filed_list, list):
        param_dict = {k: v for k, v in param_dict.copy().items() if not k in remove_filed_list}
...
def parse_data(request, **kwargs):
    ...
    if method == "GET":
        ...
        data = request.data
    return format_params_handle({k: v for k, v in data.items()}, **kwargs)
