...
class Binomial(Distribution):
    ...
    def plot_bar(self):
        ...
        Returns:
            None
        """
        ptl.bar(x = ["0", "1"], height =[(1-self.p)*self.n, self.p*self.n])
    ...
    def pdf(self, k):
        ...
        """
        n_choose_x = math.factorial(self.n)/(math.factorial(k) * math.factorial(self.n-k))
        binomial_pdf = n_choose_x * (self.p**k)*((1-self.p)**(self.n-k))
        return binomial_pdf
    ...
    def plot_bar_pdf(self):
        ...
        """
        x = [point for point in range(self.n + 1)]
        y = []
        for i in x:
            y.append(x.pdf(point))
        plt.bar(x, y)
