...
class AutoProcSaveDeleteModelAdmin(admin.ModelAdmin):
    """
    重写 save_odel和delete_model,数据更新自动删除缓存
    """
    ...
    def get_actions(self, request):
        ...
        actions = super(AutoProcSaveDeleteModelAdmin, self).get_actions(request)
    ...
    def save_model(self, request, obj, form, change):
        ...
        print('删除缓存前', cache.get(model_name))
        cache.delete(model_name)
        print('准备删除缓存')
        print('删除缓存后', cache.get(model_name))
        obj.save()
