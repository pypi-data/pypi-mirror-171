...
def mock_no_msgs(time, messages_sent):
    return [], None


def mock_msg_from_agent_0(time, messages_sent):
...
def mock_error(time, messages_sent):
    """
...
def mock_init_and_create_client0_and_client1_model(communication_simulator):
    ...
    communication_simulator._step_size = USED_STEP_SIZE
...
def test_receive_messages_from_omnetpp_info_msg(monkeypatch):
    ...
    answers, next_step = communication_simulator.receive_messages_from_omnetpp(time=0,
                                                                               messages_sent=False)
...
def test_receive_messages_from_omnetpp_transmission_error(monkeypatch):
    ...
    communication_simulator.receive_messages_from_omnetpp(time=0, messages_sent=False)
...
def test_receive_messages_from_omnetpp_disconnect(monkeypatch):
    ...
    # directly returns
    answers, next_step = communication_simulator.receive_messages_from_omnetpp(time=0, messages_sent=False)
...
def test_receive_messages_from_omnetpp_max_advance(monkeypatch):
    ...
    with pytest.raises(ValueError):
        communication_simulator.receive_messages_from_omnetpp(time=200, messages_sent=False)
...
def test_check_msgs():
    ...
    msg_1 = msg_group.infrastructure_messages.add()
    msg_1.msg_type = InfrastructureMessage.MsgType.RECONNECT
    ...
    with pytest.raises(RuntimeError):
        communication_simulator.check_msgs(msgs, time=3, reached_until=False)

    # sim time is not further than last message time, therefore no
    # error is raised
    communication_simulator.check_msgs([msg, msg_1], time=0, reached_until=False)
    ...
    msgs = [{}, {}]
    with pytest.raises(ValueError):
        communication_simulator.check_msgs(msgs, time=1, reached_until=False)
