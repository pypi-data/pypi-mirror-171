...
# |  PSQLConnectionError  |
# -------------------------
class PSQLConnectionError(Errors):
...
# -------------------------------
class ConfigPermissionError(Errors):
...
# --------------------------
class PreExecutionError(Errors):
...
# |  BUILD ERRORS  |
# ------------------
class BuildError(Errors):
    """Error raised when something wrong when we build the request."""


class DeleteError(Errors):
    """Raised when we try to make a delete with a predicat and armagedon == True."""

    def __init__(self, other_cmd: str) -> None:
        """Initialize a DeleteError."""
        super().__init__(
            "You can't make a delete with "
            f"a {other_cmd} and armagedon set at True. You need to choose.",
        )


class LengthMismatchError(Errors):
...
class NeedWhereError(Errors):
    """Raised when we try to make a delete without armagedon and without where."""
    ...
    def __init__(self) -> None:
        ...
        super().__init__(
            ...
            "the parameter `armagedon` to say 'I want to delete all my table'.",
        )


class TooManyReturnValueError(TypeError, Errors):
...
# -----------------------------
class UnknownCommandTypeError(TypeError, Errors):
