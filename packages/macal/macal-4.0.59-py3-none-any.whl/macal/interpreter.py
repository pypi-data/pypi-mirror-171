...
class Interpreter:
    ...
    def InterpretAssignToIndexedVar(self, var: variable.Variable, idx: ast_expr.Expr, value: value_item.ValueItem, scope: mscope.Scope):
        if var.Value.Type not in [types.VariableTypes.Array, types.VariableTypes.Record, types.VariableTypes.String]:
            raise exceptions.RuntimeError(f'Type error, index not supported on {var.Value.Type}. ({var.Name}).', idx.Token.Location, self.filename)
        val = var.Value
        if val.Type == types.VariableTypes.Nil and val.Value != types.VariableTypes.Nil:
            raise exceptions.RuntimeError(f'AssignIndexed Type error, val is not nil, but type is. ({var.Name}).', idx.Token.Location, self.filename)
        pred = var.Value
        expr = idx
        vind = []
        ...
        for ind in vind:
            ...
            index_as_str = f'{index_as_str}[{i.Value}]'
            if val.Type not in [types.VariableTypes.Array, types.VariableTypes.Record, types.VariableTypes.String]:
                if not(val.Type == types.VariableTypes.Nil and pred != val and pred.Type == types.VariableTypes.Record):
                    raise exceptions.RuntimeError(f'Index type ({val.Type}) error. ({var.Name}{index_as_str})', ind.Token.Location, self.filename)    
            if ind.ExprType == types.ExprTypes.NewArrayIndex:
                if val.Type != types.VariableTypes.Array:
                    raise exceptions.RuntimeError(f'Append not supported. ({var.Name}{index_as_str})', ind.Token.Location, self.filename)
            ...
            if val.Type == types.VariableTypes.Record:
                if i.Value not in val.Value:
                    if cnt == lng -1:
                        ...
                        raise exceptions.RuntimeError(f'Index not found. ({var.Name}{index_as_str}).', ind.Token.Location, self.filename)
            elif val.Type == types.VariableTypes.Array or val.Type == types.VariableTypes.String:
                if i.Type != types.VariableTypes.Int:
                    raise exceptions.RuntimeError(f'Invalid index type ({i.Type}). ({var.Name}{index_as_str}).', ind.Token.Location, self.filename)
                if i.Value < 0 or i.Value >= len(val.Value):
                    raise exceptions.RuntimeError(f'Index out of range. ({var.Name}{index_as_str})', ind.Token.Location, self.filename)
            elif val.Type == types.VariableTypes.Nil and pred.Type == types.VariableTypes.Record:
                if cnt == lng -1:
                    val.Value[i.Value] = value_item.ValueItem().NilValue()
                    return
                else:
                    raise exceptions.RuntimeError(f'Invalid index ({i.Type}) on nil value. ({var.Name}{index_as_str}).', ind.Token.Location, self.filename)
            pred = val
            val = val.Value[i.Value]
            if val.Type == types.VariableTypes.Nil and val.Value != types.VariableTypes.Nil:
                raise exceptions.RuntimeError(f'AssignToIndexed, val is not nil, but type is. ({var.Name}{index_as_str}).', idx.Token.Location, self.filename)
            cnt+=1

        if value.Type == types.VariableTypes.Nil and value.Value != types.VariableTypes.Nil:
            raise exceptions.RuntimeError(f'AssignToIndexed VALUE is not nil, but type is. ({var.Name}{index_as_str}).', idx.Token.Location, self.filename)
    ...
    def InterpretAssign(self, instruction: ast_assignment.Assignment, scope: mscope.Scope) -> None:
        ...
        if value.Type == types.VariableTypes.Nil and value.Value != types.VariableTypes.Nil:
            raise exceptions.RuntimeError(f'Value type is nil, but value is not. ({instruction.Variable}).', instruction.Token.Location, self.filename)
    ...
    def InterpretIndexedVariableExpression(self, var: variable.Variable, idx: ast_expr.Expr, scope: mscope.Scope)-> value_item.ValueItem:
        ...
        for ind in vind:
            ...
            index_as_str = f'{index_as_str}[{i.Value}]'
            if val.Type not in [types.VariableTypes.Array, types.VariableTypes.Record, types.VariableTypes.String]:
                raise exceptions.RuntimeError(f'Index type ({val.Type}) not supported. ({var.Name}{index_as_str}).', ind.Token.Location, self.filename)
            if ind.ExprType == types.ExprTypes.NewArrayIndex:
                raise exceptions.RuntimeError('Append not supported. ({var.Name}{index_as_str}).', ind.Token.Location, self.filename)
            if val.Type == types.VariableTypes.Record:
                if i.Value not in val.Value:
                    raise exceptions.RuntimeError(f'Index not found. ({var.Name}{index_as_str}).', ind.Token.Location, self.filename)
            elif val.Type == types.VariableTypes.Array or val.Type == types.VariableTypes.String:
                if i.Type != types.VariableTypes.Int:
                    raise exceptions.RuntimeError(f'Invalid index type ({i.Type}), Int index type required. ({var.Name}{index_as_str}).', ind.Token.Location, self.filename)
                if i.Value < 0 or i.Value >= len(val.Value):
                    raise exceptions.RuntimeError(f'Index out of range. ({var.Name}{index_as_str}).', ind.Token.Location, self.filename)
            else:
                raise exceptions.RuntimeError(f'Type error, index not supported on {val.Type}. ({var.Name}{index_as_str}).', ind.Token.Location, self.filename)
            if val.Type == types.VariableTypes.String:
                val = value_item.ValueItem().SetFromMacal(val.Token, val.Type, val.Value[i.Value])
            else:
                val = val.Value[i.Value]
        if expr.Right is not None and expr.Right.ExprType == types.ExprTypes.NewArrayIndex:
            raise exceptions.RuntimeError(f'Append not supported. ({var.Name}{index_as_str}).', expr.Right.Token.Location, self.filename)
        return val
    ...
    def RunFunctionCall(self, instruction: ast_function_call, args: typing.List[ast_expr.Expr], tok: token.LexToken, scope: mscope.Scope) -> value_item.ValueItem:
        ...
        if returnVar.Value.Type == types.VariableTypes.Nil and returnVar.Value.Value != types.VariableTypes.Nil:
            raise exceptions.RuntimeError(f"ExtFunction ({name}) returned nil type with a wrong value ({returnVar.Value}).", tok.Location, self.filename)
    ...
    def checkDataArray(self, arr, msg):
        ...
        if arr.Type == types.VariableTypes.Nil and arr.Value != types.VariableTypes.Nil:
            raise exceptions.RuntimeError(f'checkDataArray ({msg}), arr.Type is nil but arr.Value is not.', None, self.filename)
        if arr.Type == types.VariableTypes.Array and len(arr.Value) > 0:
            for rec in arr.Value:
                ...
                if rec.Type == types.VariableTypes.Nil and rec.Value != types.VariableTypes.Nil:
                    raise exceptions.RuntimeError(f'checkDataArray ({msg}), rec.Type is nil but rec.Value is not.', None, self.filename)
    ...
    def InterpretSelect(self, instruction: ast_select.Select, scope: mscope.Scope) -> None:
        ...
        self.checkDataArray(into, f'into after EvaluateIntoVariable, {instruction.Into}')
        if into is None:
            ...
            into = intoVar.Value      
            self.checkDataArray(into, 'into from new intoVar after into was none')
        ...
        self.checkDataArray(data, 'data after apply field filters')
        ...
        if data is not None and data.Type == types.VariableTypes.Array and len(data.Value) == 1:
            if isinstance(data.Value[0], value_item.ValueItem):
                ...
                raise exceptions.RuntimeError(f'Debug Select (array of one 1): data.Value[0] is NOT ValueItem but it should!', data.Token.Location, self.filename)              
        ...
        if data is not None and data.Type == types.VariableTypes.Array and len(data.Value) == 1:
            if isinstance(data.Value[0], value_item.ValueItem):
                ...
                raise exceptions.RuntimeError(f'Debug Select (array of one 2): data.Value[0] is NOT ValueItem but it should!', data.Token.Location, self.filename)
    ...
    def SetIndexedIntoVariableValue(self, var: variable.Variable, idx: ast_expr.Expr, data: value_item.ValueItem, scope: mscope.Scope)-> None:
        if var.Value.Type not in [types.VariableTypes.Array, types.VariableTypes.Record, types.VariableTypes.String]:
            raise exceptions.RuntimeError(f'Type error, index not supported on {var.Value.Type}', idx.Token.Location, self.filename)    
        expr = idx
        vind = []
        while expr.ExprType != types.ExprTypes.VariableIndexStart:
        ...
        for ind in vind:
            ...
            if val.Type == types.VariableTypes.Record:
                if i.Value not in val.Value:
                    raise exceptions.RuntimeError(f'Index ({i.Value}) not found. ({var.Name}{index_as_str}).', ind.Token.Location, self.filename)
            elif val.Type == types.VariableTypes.Array or val.Type == types.VariableTypes.String:
                if i.Type != types.VariableTypes.Int:
                    raise exceptions.RuntimeError(f'Invalid index type ({i.Type}), Int index type required. ({var.Name}{index_as_str}).', ind.Token.Location, self.filename)
                if i.Value < 0 or i.Value >= len(val.Value):
                    raise exceptions.RuntimeError(f'Index ({i.Value}) is out of range. ({var.Name}{index_as_str}).', ind.Token.Location, self.filename)
            else:
                raise exceptions.RuntimeError(f'Type error, index not supported on {val.Type}. ({var.Name}{index_as_str}).', ind.Token.Location, self.filename)
            finalIndex = i.Value
            finalVal = val
    ...
    def EvaluateIntoVariable(self, expr: ast_expr.Expr, rec: value_item.ValueItem, scope: mscope.Scope) -> value_item.ValueItem:
        ...
        if var is None:
            return None
        if expr.Left is None:
            if var.Value.Value != types.VariableTypes.Nil and var.Value.Type == types.VariableTypes.Nil:
                raise exceptions.RuntimeError(f'Eval Into Var, val.Value is not nil, val.Type is nil. ({var.Name}).', expr.Token.Location, self.filename)
            return var.Value
        if expr.Left.ExprType == types.ExprTypes.VariableIndex:
    ...
    def EvaluateDatafieldIndexedVariable(self, var: variable.Variable, idx: ast_expr.Expr, rec: dict, scope: mscope.Scope)-> value_item.ValueItem:
        if var.Value.Type not in [types.VariableTypes.Array, types.VariableTypes.Record, types.VariableTypes.String]:
            raise exceptions.RuntimeError(f'Type error, index not supported on {var.Value.Type}', ind.Token.Location, self.filename)    
        expr = idx
        vind = []
        while expr.ExprType != types.ExprTypes.VariableIndexStart:
            if expr.Right.ExprType == types.ExprTypes.NewArrayIndex:
                raise exceptions.RuntimeError(f'Append not supported. ({var.Name})', ind.Token.Location, self.filename)
            vind.insert(0, expr.Right)
            expr = expr.Left
        val = var.Value
        index_as_str = ''
        ...
        if expr.Right is not None and expr.Right.ExprType == types.ExprTypes.NewArrayIndex:
            raise exceptions.RuntimeError(f'Append not supported. ({var.Name}{index_as_str}).', ind.Token.Location, self.filename)
        if val.Value != types.VariableTypes.Nil and val.Type == types.VariableTypes.Nil:
            raise exceptions.RuntimeError(f'EvalIndexed, val.Value is not nil, val.Type is nil. ({var.Name}{index_as_str}).', ind.Token.Location, self.filename)
        return val
