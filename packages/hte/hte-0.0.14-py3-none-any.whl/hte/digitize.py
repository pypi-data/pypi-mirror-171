...
class Book:
    ...
    def OCRGroupingTaxeringskalender(self, df_input, from_page=None, to_page=None):
        ...
        for i in range(length-2, -1, -1):
            ...
            if text.at[i, "end"] == "-":
                text.at[i, 'DUMMY'] = 1
            if text.at[i, "end"] == "—" and re.search("^[0-9]{3,}$", text.at[i+1, "text"]):
                text.at[i, 'DUMMY'] = 1
            if re.search("^—[ ]{0,1}[0-9]{3,}$", text.at[i+1, "text"]):
                text.at[i, 'DUMMY'] = 1
            if text.at[i, "end"] == ",":
                text.at[i, 'DUMMY'] = 1
            if text.at[i, "end"] == ",":
    ...
    def RegexTaxeringskalender(self, df):
        ...
        extract = csv['text'].str.extract(r"^(.*?) ([a-xæøåäö., ]{3,})[ ]*([A-ZÆØÅÄÖ]*[^0-9]*?)([0-9]*)[—\- ]{1,3}([0-9]{1,})$")
        ...
        del csv
        extract = csv.extract(['Unnamed: 0', 'level', 'page_num', 'par_num',
                               'line_num', 'word_num', 'width', 'conf',
                               'text_part'],
                              axis=1, errors='ignore')
