...
class DisplacementOperator(Node):
    ...
    >>> graph.displacement_operator(1j, 2, name="displacement_op")
    >>> result = qctrl.functions.calculate_graph(graph=graph, output_node_names=["displacement_op"])
    >>> result.output["displacement_op"]["value"]
    ...
    >>> graph.displacement_operator([1j, 3], 2, name="displacement_op_batch")
    >>> result = qctrl.functions.calculate_graph(
    ...     graph=graph, output_node_names=["displacement_op_batch"]
    ... )
    >>> result.output["displacement_op_batch"]["value"]
...
class SqueezeOperator(Node):
    ...
    >>> graph.squeeze_operator(1j, 3, name="squeeze_op")
    >>> result = qctrl.functions.calculate_graph(graph=graph, output_node_names=["squeeze_op"])
    >>> result.output["squeeze_op"]["value"]
    ...
    >>> graph.squeeze_operator([1j, 3], 3, name="squeeze_op_batch")
    >>> result = qctrl.functions.calculate_graph(
    ...     graph=graph, output_node_names=["squeeze_op_batch"]
    ... )
    >>> result.output["squeeze_op_batch"]["value"]
    array([[[ 0.7602446 +0.j        ,  0.        +0.j        ,0.        -0.64963694j],
            [ 0.        +0.j        ,  1.        +0.j        ,0.        +0.j        ],
            [ 0.        -0.64963694j,  0.        +0.j        ,0.7602446 +0.j        ]],
