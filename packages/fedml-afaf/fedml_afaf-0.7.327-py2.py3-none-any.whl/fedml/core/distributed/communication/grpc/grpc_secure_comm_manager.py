...
class GRPCSecureCommManager(BaseCommunicationManager):
    def __init__(
        ...
        self.host = host
        self.port = str(port)
        self.ca_credentials = ca_credentials
        self._topic = topic
        ...
        if not check_tls_name:
            self.opts.append(('grpc.ssl_target_name_override', "None"))
        ...
        self.grpc_server.add_secure_port("{}:{}".format("0.0.0.0", port), credentials)

        self.grpc_server.start()
        self.is_running = True
        logging.info("secure grpc server started. Listening on port " + str(port))
    ...
    def send_message(self, msg: Message):
        ...
        channel = grpc.secure_channel(channel_url, self.ca_credentials, options=self.opts)
