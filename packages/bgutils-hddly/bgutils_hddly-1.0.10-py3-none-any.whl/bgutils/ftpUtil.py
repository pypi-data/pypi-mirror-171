...
from entity.entSftpFile import entSftpFile
...
class ftpUtil:
    ...
    def putfile_stud(self,local_path,prj_id,stud_id,no_file,file_type):
        # # 要传输文件的路径
        # filepath = "./myname.jpg"
        # # 上传后的传输文件的文件名
        ...
        try:
            ...
            sftp.chdir(self.__remotedir_stud)
        ...
        try:
            #记录文件列表
            ent = entSftpFile(local_path,self.__url_head+remote_file,stud) #filename,url,stud
            self.__mutil.sftp_file_ins(ent)
        except Exception as ex:
