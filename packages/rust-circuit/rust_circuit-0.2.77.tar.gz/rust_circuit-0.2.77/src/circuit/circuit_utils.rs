...
use super::{
    apply_fn_cache, deep_map_op_pre_new_children, deep_map_unwrap, parse_compiler_repr_bijection,
    visit_circuit, visit_circuit_fallable, visit_circuit_with_parents, ArrayConstant, Circuit,
    CircuitConstructionError, CircuitNode, CircuitRc, HashBytes, Index,
};
...
pub fn subcircuits(circuit: &Circuit) -> HashSet<CircuitRc> {
    let mut result: HashSet<CircuitRc> = HashSet::new();
    visit_circuit(circuit, &mut |x: &Circuit| {
        result.insert(x.clone().rc());
    });
    result
}
    ...
    F2: Fn(CircuitRc, &Vec<T>) -> CircuitRc,
{
    ...
    deep_map_unwrap(&circuit, |c| {
        make_circuit(
            c.clone().rc(),
            passed_down.get(&c.clone().rc()).unwrap_or(&vec![]),
        )
    })
}
