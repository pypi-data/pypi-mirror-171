...
class Target:
    ...
    def __repr__(self) -> str:
        return f"Target('{self.name}', {self.weight}, {self.ineq})"
...
class FeatureDefinition:
    ...
    def _update(self) -> None:
        ...
        self.punits = {p.name: p.unit for p in self.parameters}
        ...
        assert sum(self.weights.values()) == 1, "Objective weights must sum up to 1."
...
class Surrogate(ABC):
    ...
    def __call__(
        ...
        y = data[
            [
                tname
                for tname, isobjective, isconstr in zip(
                    features.tnames, features.weights.values(), features.ineqs.values()
                )
                if isobjective != 0 or isconstr is not None
            ]
        ].values
...
class NewOptimization(ABC):
    def __init__(
        ...
        self.nvar = features.nvar
        self.nobj = len([f for f in features.weights.values() if f != 0])
        self.n_ieq_constr = len([f for f in features.ineqs.values() if f is not None])
        self.lbs = list(features.lbs.values())
        self.ubs = list(features.ubs.values())
        self.weights = list(features.weights.values())
...
class Problem(ElementwiseProblem):
    ...
    def _evaluate(self, x, out, *args, **kwargs) -> None:
        ...
        F = [w * res for w, res in zip(self.weights, results) if w != 0]
        G = [res - ineq for res, ineq in zip(results, self.ineqs) if ineq is not None]
        out["F"] = F
        out["G"] = G
