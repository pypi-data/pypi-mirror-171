...
class Usecase:
    ...
    def create_trimmed_curve(self):
        items = []
        center_position = self.file.createIfcAxis2Placement3D(
            Location=self.file.createIfcCartesianPoint(polygon.center),
            Axis=self.file.createIfcDirection(polygon.normal),
        )
        radius = 42
        basis_curve = self.file.createIfcCircle(center_position, radius)
        self.file.createIfcTrimmedCurve(basis_curve, trim1, trim2, sense, master)
        pass
    ...
    def create_annotation2d_representation(self):
        if self.settings["ifc_representation_class"] == "IfcTrimmedCurve":
            items = self.create_trimmed_curve()
        elif isinstance(self.settings["geometry"], bpy.types.Mesh) and len(self.settings["geometry"].polygons):
