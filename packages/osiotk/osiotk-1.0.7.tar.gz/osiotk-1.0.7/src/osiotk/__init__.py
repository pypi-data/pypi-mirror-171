...
def __mk_filetree(__ft: str):
    ...
    def has_subfiles(__name: str) -> bool:
        result = False
        for path in ft_paths:
            if path.startswith(f"{__name}/") and path != __name:
                result = True
                break
        return result
    ...
    dirpaths = set(
        path
        for path in ft_paths
        if has_subfiles(path) and not "." in __basename(path)
    )

    for path in ft_paths:
        (__mkdir if path in dirpaths else __mkfile)(path)
...
def mk_filetree(__ft: str):
    return __mk_filetree(__ft)
