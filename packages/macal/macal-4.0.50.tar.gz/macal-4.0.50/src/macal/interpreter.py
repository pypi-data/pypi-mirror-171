...
class Interpreter:
    ...
    def RunFunctionCallFromExpr(self, name: str, args: typing.List[ast_expr.Expr], tok: token.LexToken, scope: mscope.Scope) -> value_item.ValueItem:
        ...
        return self.RunFunctionCall(self, instruction, args, tok, scope)
    ...
    def RunFunctionCall(self, instruction: ast_function_call, args: typing.List[ast_expr.Expr], tok: token.LexToken, scope: mscope.Scope) -> value_item.ValueItem:
        ...
        if returnVar.Type == types.VariableTypes.Nil and returnVar.Value.Value != types.VariableTypes.Nil:
            raise exceptions.RuntimeError(f"ExtFunction ({name}) returned nil type without wrong value ({returnVar.Value}).", tok.Location, self.filename)
