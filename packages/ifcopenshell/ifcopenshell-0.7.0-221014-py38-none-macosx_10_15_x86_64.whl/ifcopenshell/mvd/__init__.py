...
class template(object):
    ...
    def parse_rule(self, root, visited=None):
        def visit(node, prefix="", visited=None):
                    ...
                    def child_has_ruleid_or_prefix(node):
                        if type(node).__name__ == "Element":
                            ...
                            for n in node.childNodes:
                                if child_has_ruleid_or_prefix(n):
                                    return True
            ...
            elif node.localName == "EntityRules":
                pass
            elif node.localName == "AttributeRules":
                pass
            elif node.localName == "Rules":
                pass
            elif node.localName == "Constraints":
                pass
            elif node.localName == "References":
                pass
            elif node.localName == "Definitions":
                return
            elif node.localName == "SubTemplates":
                return  # @todo perhaps just traverse them?
            else:
                raise ValueError(node.localName)

            def _(n):
                for subnode in n.childNodes:
                    if not isinstance(subnode, Element):
                        continue
                    for x in visit(subnode, p, visited=visited):
                        yield x
