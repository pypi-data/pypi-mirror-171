...
class StatorCoreTemperOverLimitDiagnosis(MultipleThresholds_GreaterEqualTo_LesserThan):
    def __init__(self, *args, **kwargs):
        """
        定子铁芯温度超限判断,单位均为℃

        :math:`T < 110, 定子铁芯运行温度不超限`

        :math:`110 ≤ T < 125, 定子铁芯运行温度超限一级报警`

        :math:`T ≥ 125, 定子铁芯运行温度超限二级报警`


        [1] 参数
        ----------
        thresholds:
            list[float],二级报警限,默认[110, 125]
        truthTable:
            dict,报警真值表,形如:
                {
        ...
        >>>     "001": {"dfem_evidence": "[定子铁芯温度]实测值(measured)二级超限,阈值threshold", "dfem_code": "SP000032"},
        >>> }
        >>> obj = StatorCoreTemperOverLimitDiagnosis([110, 125], truthTable)
