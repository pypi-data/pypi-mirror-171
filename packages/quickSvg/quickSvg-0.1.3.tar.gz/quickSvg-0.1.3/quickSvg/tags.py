# copy from https://github.com/evtn/soda/blob/lord/soda
...
class MetaTag(type):
    def __getattr__(self, attr: str) -> "Tag":
        return Tag(attr.lstrip('_').replace("__", ":").replace("_", "-"))
...
class Tag(metaclass=MetaTag):
    ...
    def __init__(
        ...
        self.attributes = {k.lstrip('_').replace("__", ":").replace("_", "-"): v for k, v in attributes.items()}
