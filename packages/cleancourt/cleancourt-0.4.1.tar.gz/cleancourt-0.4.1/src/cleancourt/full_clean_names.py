...
from src.cleancourt.separate_parent_company import separate_company_names
...
def full_clean_names1(messy_names):
    ...
    nameA, nameB = separate_company_names(messy_names)

    df['clean_names_1a'] = [clean_data(x) for x in tqdm(nameA)]
    df['clean_names_1b'] = [clean_data(x) for x in tqdm(nameB)]


    # Parse the people names and format
    logger.info("PARSING PEOPLE NAMES")
    duplicates_dict_peopleA = format_people_names(df['clean_names_1a'])
    duplicates_dict_peopleB = format_people_names(df['clean_names_1b'])


    # Merge names into df
    logger.info("MERGING PEOPLE NAMES")
    df['formatted_people_namesA'] = df.apply(lambda x: map_values(x.clean_names_1a, duplicates_dict_peopleA), axis=1)
    df['formatted_people_namesB'] = df.apply(lambda x: map_values(x.clean_names_1b, duplicates_dict_peopleB), axis=1)



    # Parse company names
    logger.info("PARSING COMPANY NAMES")
    duplicates_dict_companyA = link_company_names(df['clean_names_1a'])
    duplicates_dict_companyB = link_company_names(df['clean_names_1b'])

    # Merge company names into df
    logger.info("MERGING COMPANY NAMES")
    df['formatted_company_namesA'] = df.apply(lambda x: map_values(x.clean_names_1a, duplicates_dict_companyA), axis=1)
    df['formatted_company_namesB'] = df.apply(lambda x: map_values(x.clean_names_1b, duplicates_dict_companyB), axis=1)

    # Create column of all company and people names
    logger.info("ADDING ALL NAMES TO DF")
    df['clean_names_2a'] = df.apply(lambda x: combine_columns(x.clean_names_1a, x.formatted_people_namesA, x.formatted_company_namesA), axis=1)
    df['clean_names_2b'] = df.apply(lambda x: combine_columns(x.clean_names_1b, x.formatted_people_namesB, x.formatted_company_namesB), axis=1)
