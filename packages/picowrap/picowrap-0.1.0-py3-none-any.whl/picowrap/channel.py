...
class Channel:
    ...
    def get_values(self):
        ...
        if time_interval_ns * 10**-9 != self.picoscope.sampling:
            print(
                f"WARNING sampling ({self.picoscope.sampling})",
                f"!= real time_interval ({time_interval_ns * 10**-9}) ",
                f"Error = {self.picoscope.sampling-time_interval_ns * 10**-9} [s]",
            )
