...
import os, urllib, multiprocessing, argparse
...
# Get env variable
RECHTSPRAAK_METADATA_API_BASE_URL = os.getenv('RECHTSPRAAK_METADATA_API_BASE_URL')
...
def get_rechtspraak_metadata():
    ...
    csv_files = read_csv(DIR_DATA_RECHTSPRAAK, "metadata")
    ...
    if len(csv_files) > 0:
        ...
        for f in csv_files:
            ...
            if args.save_file == 'y':
                file_check = Path(DIR_DATA_RECHTSPRAAK + "/" + f.split('/')[-1][:len(f.split('/')[-1]) - 4] + "_metadata.csv")
            ...
            if args.save_file == 'y':           # Save file as CSV if selected
                # Save CSV file
                ...
                Path(DIR_DATA_RECHTSPRAAK).mkdir(parents=True, exist_ok=True)

                rechtspraak_df.to_csv(DIR_DATA_RECHTSPRAAK + "/" +
