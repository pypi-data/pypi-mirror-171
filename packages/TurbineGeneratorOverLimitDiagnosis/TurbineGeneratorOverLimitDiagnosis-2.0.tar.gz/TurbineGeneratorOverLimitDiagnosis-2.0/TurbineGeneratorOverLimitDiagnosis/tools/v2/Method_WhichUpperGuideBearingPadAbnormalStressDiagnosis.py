...
class WhichUpperGuideBearingPadAbnormalStressDiagnosis:
    def __init__(self, **kwargs):
        """
        第i号上导轴瓦受力异常评估

        :math:`T < 0.035, 上导轴瓦受力无异常`

        :math:`T ≥ 0.035, 第i号上导轴瓦受力异常`
        ...
        truthTable:
            dict,报警真值表,形如:
                {
                    "10": {"dfem_evidence": "[第(i)号上导轴瓦受力]温度偏差计算值(measured)正常,阈值threshold", "dfem_code": ""},
                    "01": {"dfem_evidence": "[第(i)号上导轴瓦受力]温度偏差计算值(measured)异常,阈值threshold", "dfem_code": "SP0006(i))"},
                }
        roundingFormat:
            str, 数值精度格式,默认"%2.2f"
        ...
        kwargs["truthTable"] = kwargs["truthTable"] if "truthTable" in kwargs.keys() else {
            "10": {"dfem_evidence": "[第(i)号上导轴瓦受力]正常", "dfem_code": ""},
            "01": {"dfem_evidence": "[第(i)号上导轴瓦受力]温度偏差计算值(measured)异常,阈值threshold", "dfem_code": "SP0006(i)"},
        }
    ...
    def diagnosis(self, tempers: dict) -> list:
        ...
        返回形如:
            {
                'dfem_evidence': [
                    '[第1号上导轴瓦受力]温度偏差计算值(1.39)异常,阈值0.04',
                    ...,
                    '[第3号上导轴瓦受力]温度偏差计算值(0.57)异常,阈值0.04'
                ],
                'dfem_code': [
                    'SP000601)',
                    ...,
