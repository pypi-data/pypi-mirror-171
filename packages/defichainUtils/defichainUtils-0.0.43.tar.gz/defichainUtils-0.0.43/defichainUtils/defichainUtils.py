...
def processCustomTransactionOnChain(block,name,parameters:dict,pool:dict):
    ...
    if name == 'addpoolliquidity':
        ...
        (reserveAChange,reserveBChange,swapped,commisionAmount,dexFeeInAmount,dexFeeOutAmount) = chain.poolSwap(block,parameters['fromAmount'],parameters['isForward'],pool['reserveA'],pool['reserveB'],parameters['commission'],parameters['dexFeeInPct'],parameters['dexFeeOutPct'])
        return {
            'success': 1,
