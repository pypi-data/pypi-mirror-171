...
class Interpreter:
    ...
    def EvaluateDatafieldVariable(self, expr: ast_expr.Expr, rec: value_item.ValueItem, scope: mscope.Scope) -> value_item.ValueItem:
        if expr.Token.Lexeme in rec.Value:
            if rec.Value[expr.Token.Lexeme] != types.VariableTypes.Nil and rec.Value[expr.Token.Lexeme].Type == types.VariableTypes.Nil:
                raise exceptions.RuntimeError(f'EvalDataFieldIndexed, val.Value is not nil, val.Type is nil.', expr.Token.Location, self.filename)
