...
class keras_predictor:
    ...
    def build_model(self, trainset_shape, model_name='Keras model', model_file=None):
        ...
        if model_file is not None:
            ...
            else: raise ValueError('File does not exist:', model_file)
    ...
    def tpu_model(self, shape, model_file):
        """
        ...
        with strategy.scope(): # Change Keras model to TPU form
            model = self.build_model(shape, model_file) # Build the model # Use model.summary() to show the model structure
        return model
    ...
    def ensemble_keras_predict(self, data=None, show=False, plot=False, save=False, **kwargs):
        ...
        **kwargs        - any parameters of self.keras_predict()       
        ...
        df_redecom, df_redecom_list = redecom(data, show, self.DECOM_MODE, self.INTE_LIST, self.REDECOM_LIST, self.VMD_PARAMS)
        ...
        **kwargs           - any parameters of self.keras_predict()
        ...
        if transfer_learning: 
            ...
            if predict_method == 'Single': model = change_name(model_name)+'_model.h5'
                ...
                if self.REDECOM_LIST is not None and 'vmd' not in str(self.REDECOM_LIST).lower(): print('Warning! It is recommended to use VMD or OVMD in REDECOM_LIST. Otherwise, a different number of decomposed IMFs per run may lead to errors.')
                model = pd.DataFrame(index=[0])
                if predict_method == 'Ensemble': 
                    model = change_name(model_name)+'_model.h5'
                    if self.REDECOM_LIST is not None: df_redecom, df_redecom_list = redecom(data, False, self.DECOM_MODE, self.INTE_LIST, self.REDECOM_LIST)
                elif predict_method == 'Respective': 
                    df_redecom, df_redecom_list = redecom(data, False, self.DECOM_MODE, self.INTE_LIST, self.REDECOM_LIST)
        ...
        for i in range(self.FORECAST_LENGTH):
            ...
            time1 = time.time()
            # with self.HiddenPrints():
            if i != 0 and model is not None: self.KERAS_MODEL = model
            self.TARGET = data[:-self.FORECAST_LENGTH+i]
            if predict_method == 'Single': df_result = self.single_keras_predict(data=self.TARGET, **kwargs)
            if predict_method == 'Ensemble': df_result = self.ensemble_keras_predict(data=self.TARGET, **kwargs) 
            if predict_method == 'Respective': df_result = self.respective_keras_predict(data=self.TARGET, **kwargs)
            if predict_method == 'Hybrid': df_result = self.hybrid_keras_predict(data=self.TARGET, show=True, **kwargs)
            time2 = time.time()
            print('taking time: %.3fs'%(time2-time1))
            df_result['Run'] = i
        ...
        df_pred_result['real'] = self.TARGET[-self.FORECAST_LENGTH:]
        if predict_method == 'Single' or predict_method == 'Ensemble': df_pred_result['predict'] = df_next_result['next_pred']
        else: df_pred_result['predict'] = df_next_result.loc[df_next_result['IMF'] == 'Final']['next_pred']
