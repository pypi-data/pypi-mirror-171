...
@mock.patch(
    ...
    side_effect=Exception("*** Tuxsuite not build repo"),
)
...
def test_tux_console_plan_error(mock_run, squad_group_good, squad_project_good):
    ...
    mock_run.return_value = build
    assert tux_console_plan(None, PLAN, squad_group_good, squad_project_good) == 1
...
def test_squad_sha_request(mock_session):
    ...
    squad_sha = squad_metadata_request("tuxtrigger")
...
def test_squad_sha_request_fail(mock_session):
    ...
    with pytest.raises(Exception) as exc:
        squad_metadata_request("tuxtrigger")
    assert "SQUAD response error - (get project id) 404" in str(exc)

    assert squad_metadata_request(None) == (None, None)


@mock.patch("tuxtrigger.builder.squad_metadata_request")
def test_compare_squad_sha(mock_sha):
    mock_sha.return_value = ("1234", "1234")
    assert compare_squad_sha("tuxtrigger", "1234") is False
    assert compare_squad_sha("tuxtrigger", "5678") is True


def test_compare_squad_sha_none():
    assert compare_squad_sha(None, "1234") is True
